...
/**
 * Immutable representation of an RSK address. It is a simple wrapper on the raw byte[].
 *
...
public class RskAddress {
    ...
    public static final Comparator<RskAddress> LEXICOGRAPHICAL_COMPARATOR =
            Comparator.comparing(RskAddress::getBytes, UnsignedBytes.lexicographicalComparator());
    ...
    public RskAddress(byte[] bytes) {
        if (bytes.length != LENGTH_IN_BYTES) {
            throw new RuntimeException(
                    String.format("An RSK address must be %d bytes long", LENGTH_IN_BYTES));
        }
