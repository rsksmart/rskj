...
public class SuperDifficultyCalculator {
    ...
    private BlockDifficulty getSuperBlockDifficulty(Block curBlock, Block retargetingBlock) {
        ...
        final var multipliedCorrectionFactor = constants.getSuperBlockDifficultyCorrectionFactorShift() > 0  ?
                correctionFactor.shiftLeft(constants.getSuperBlockDifficultyCorrectionFactorShift()) :
                correctionFactor.multiply(constants.getSuperBlockDifficultyCorrectionFactor());
    ...
    private static BlockDifficulty calcSuperDifficultyWithTimeStamps(
        ...
        if (correctionFactor.compareTo(superBlockDifficultyMinCorrectionFactor) < 0) {
            ...
            correctionFactorToUse = superBlockDifficultyCorrectionFactorShift > 0 ?
                    correctionFactor.shiftRight(superBlockDifficultyCorrectionFactorShift) :
                    correctionFactor.divide(superBlockDifficultyCorrectionFactor);
        }
