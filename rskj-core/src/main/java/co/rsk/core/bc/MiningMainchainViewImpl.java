...
public class MiningMainchainViewImpl implements MiningMainchainView {
    ...
    /**
     * Given a new best header and the index of its parent rebuild the mainchain using the index as the pivot point by
     * discarding all headers subsequent to the it, setting the new header as the tip and refilling with as many are
     * needed to complete the desired height
     *
    ...
    private void addBestAndRebuildFromParent(BlockHeader bestHeader, int parentIndex) {
        List<BlockHeader> commonAncestorChain = mainchain.stream().skip(parentIndex).collect(Collectors.toList());
    ...
    private void buildMainchainFromList(List<BlockHeader> sourceList) {
        ...
        mainchain = Stream.concat(sourceList.stream(), missingHeaders.stream()).collect(Collectors.toList());
    }

    /**
     * Given a start block header and a chain length, retrieve a List of said length consisting of the start header's
     * ancestors
     *
    ...
    private void addToBlockHashesByNumberMap(BlockHeader headerToAdd) {
        ...
        if (blockHashesByNumber.containsKey(blockNumber)) {
            ...
            blockHashesByNumber.put(
                    headerToAdd.getNumber(), new ArrayList<>(Collections.singletonList(headerToAdd.getHash())));
        }
    }
