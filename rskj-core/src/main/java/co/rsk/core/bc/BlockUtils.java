...
public class BlockUtils {
    ...
    private static boolean blockInSomeBlockChain(
            Keccak256 blockHash, long blockNumber, Blockchain blockChain) {
        final List<BlockInformation> blockInformations =
                blockChain.getBlocksInformationByNumber(blockNumber);
        return blockInformations.stream()
                .anyMatch(bi -> Arrays.equals(blockHash.getBytes(), bi.getHash()));
    }

    public static Set<Keccak256> unknownDirectAncestorsHashes(
            Block block, Blockchain blockChain, BlockStore store) {
