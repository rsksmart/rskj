...
/**
 * PeerScoringManager keeps list of nodes and addresses scoring Records events by node id and
 * address Calculates good reputation by node id and address Starts punishments when the good
 * reputation is lost Alsa keeps a list of banned addresses and blocks
 *
 * <p>Created by ajlopez on 28/06/2017.
 */
public class PeerScoringManager {
    ...
    /**
     * Creates and initialize the scoring manager usually only one object per running node
     *
    ...
    public PeerScoringManager(
        ...
        this.peersByNodeID =
                new LinkedHashMap<NodeID, PeerScoring>(nodePeersSize, 0.75f, true) {
                    @Override
    ...
    public void recordEvent(NodeID id, InetAddress address, EventType event) {
        synchronized (accessLock) {
            if (id != null) {
                PeerScoring scoring =
                        peersByNodeID.computeIfAbsent(id, k -> peerScoringFactory.newInstance());
                recordEvent(scoring, event, this.nodePunishmentCalculator);
            }

            if (address != null) {
                PeerScoring scoring =
                        peersByAddress.computeIfAbsent(
                                address, k -> peerScoringFactory.newInstance());
    ...
    /**
     * Adds a network address to the set of banned addresses The address is represented in an string
    ...
    /**
     * Removes a network address from the set of banned addresses The address is represented in an
     * string If it is a block, it has a mask
     *
    ...
    /**
     * Returns the list of peer scoring information It contains the information recorded by node id
     * and by address
     *
    ...
    public List<PeerScoringInformation> getPeersInformation() {
        synchronized (accessLock) {
            List<PeerScoringInformation> list =
                    new ArrayList<>(this.peersByNodeID.size() + this.peersByAddress.size());

            list.addAll(
                    this.peersByNodeID.entrySet().stream()
                            .map(
                                    entry ->
                                            new PeerScoringInformation(
                                                    entry.getValue(),
                                                    Hex.toHexString(entry.getKey().getID())
                                                            .substring(0, 8),
                                                    "node"))
                            .collect(Collectors.toList()));
            list.addAll(
                    this.peersByAddress.entrySet().stream()
                            .map(
                                    entry ->
                                            new PeerScoringInformation(
                                                    entry.getValue(),
    ...
    /**
     * Returns the list of banned addresses, represented by a textual description The list includes
     * the banned addresses and the banned blocks
     *
    ...
    public List<String> getBannedAddresses() {
        ...
        list.addAll(
                this.addressTable.getAddressList().stream()
                        .map(entry -> entry.getHostAddress())
                        .collect(Collectors.toList()));
        list.addAll(
                this.addressTable.getAddressBlockList().stream()
    ...
    private void recordEvent(
            PeerScoring scoring, EventType event, PunishmentCalculator calculator) {
        ...
        if (!reputation && scoring.hasGoodReputation()) {
            scoring.startPunishment(
                    calculator.calculate(scoring.getPunishmentCounter(), scoring.getScore()));
        }
    }
}
