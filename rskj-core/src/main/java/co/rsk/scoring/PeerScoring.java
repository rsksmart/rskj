...
/**
 * PeerScoring records the events associated with a peer identified by node id or IP address (@see PeerScoringManager)
 * An integer score value is calculated based on recorded events. Also, a good reputation flag is calculated. The number
 * of punishment is recorded, as well the initial punishment time and its duration. When the punishment expires, the
 * good reputation is restored and most counters are reset to zero
 *
...
public class PeerScoring {
    ...
    /**
     * Records an event. Current implementation has a counter by event type. The score is incremented or decremented,
     * acoording to the kind of the event. Some negative events alters the score to a negative level, without taking
     * into account its previous positive value
     *
    ...
    /**
     * Returns the current computed score. The score is calculated based on previous event recording.
     *
     * @return An integer number, the level of score. Positive value is associated with a good reputation. Negative
     *     values indicates a possible punishment.
     */
    public int getScore() {
        try {
    ...
    /**
     * Starts the punishment, with specified duration Changes the reputation to not good Increments the punishment
     * counter
     *
