...
public class EthModuleTransactionInstant extends EthModuleTransactionBase {
    ...
    /**
     * When insta-mining we can query the transaction status and return an error response
     * immediately like Ganache. This does not apply during regular operation because queued
     * transactions are not immediately executed.
     */
    private String getReturnMessage(String txHash) {
        TransactionInfo transactionInfo =
                blockchain.getTransactionInfo(TypeConverter.stringHexToByteArray(txHash));
