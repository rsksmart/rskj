...
public class ProofEncoder {
    ...
    /**
     * Encode a list of Trie nodes into RLP-encoded hex strings using RSKj's native trie node format.
     * Each node is RLP-encoded from RSKj's binary trie serialization (RSKIP-107 format).
     *
     * Note: RSKj uses a binary trie which differs from Ethereum's hexary Merkle Patricia Trie (MPT).
     * This method only wraps RSKj's node serialization in RLP encoding and does NOT convert it to
     * Ethereum's MPT node structure. The resulting proofs are RSKj-specific and may not be verifiable
     * with standard Ethereum MPT proof verification libraries or tooling.
     *
     * @param nodes List of Trie nodes along the path (from leaf to root)
     * @return Array of RLP-encoded hex strings representing RSKj binary trie nodes
     */
