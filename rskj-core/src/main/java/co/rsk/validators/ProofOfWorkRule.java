...
public class ProofOfWorkRule implements BlockHeaderValidationRule, BlockValidationRule {
    ...
    public boolean isValid(BlockHeader header) {
        // TODO: refactor this an move it to another class. Change the Global ProofOfWorkRule to
        // AuthenticationRule.
        ...
        if (isFallbackMiningPossibleAndBlockSigned(header)) {
            boolean isValidFallbackSignature =
                    validFallbackBlockSignature(
                            constants, header, header.getBitcoinMergedMiningHeader());
        ...
        byte[] bitcoinMergedMiningCoinbaseTransactionCompressed =
                header.getBitcoinMergedMiningCoinbaseTransaction();

        if (bitcoinMergedMiningCoinbaseTransactionCompressed == null) {
            logger.warn(
                    "Compressed coinbase transaction does not exist. Header {}",
                    header.getShortHash());
            return false;
        }
        ...
        if (header.getBitcoinMergedMiningHeader() == null) {
            logger.warn(
                    "Bitcoin merged mining header does not exist. Header {}",
                    header.getShortHash());
            return false;
        }
        ...
        BigInteger bitcoinMergedMiningBlockHashBI =
                bitcoinMergedMiningBlock.getHash().toBigInteger();
        ...
        byte[] bitcoinMergedMiningCoinbaseTransactionMidstate =
                new byte[RskMiningConstants.MIDSTATE_SIZE];
        System.arraycopy(
        ...
        byte[] bitcoinMergedMiningCoinbaseTransactionTail =
                new byte
                        [bitcoinMergedMiningCoinbaseTransactionCompressed.length
                                - RskMiningConstants.MIDSTATE_SIZE_TRIMMED];
        System.arraycopy(
                bitcoinMergedMiningCoinbaseTransactionCompressed,
                RskMiningConstants.MIDSTATE_SIZE_TRIMMED,
        ...
        byte[] expectedCoinbaseMessageBytes =
                org.bouncycastle.util.Arrays.concatenate(
                        RskMiningConstants.RSK_TAG, header.getHashForMergedMining());
        ...
        List<Byte> expectedCoinbaseMessageBytesAsList =
                ListArrayUtil.asByteList(expectedCoinbaseMessageBytes);

        int rskTagPosition =
                Collections.lastIndexOfSubList(
                        bitcoinMergedMiningCoinbaseTransactionTailAsList,
        ...
        int lastTag =
                Collections.lastIndexOfSubList(
                        bitcoinMergedMiningCoinbaseTransactionTailAsList, rskTagAsList);
        ...
        if (coinbaseLength <= 64) {
            logger.warn(
                    "Coinbase transaction must always be greater than 64-bytes long. But it was: {}",
                    coinbaseLength);
            return false;
        }
        ...
        Sha256Hash bitcoinMergedMiningCoinbaseTransactionHash =
                Sha256Hash.wrapReversed(
                        Sha256Hash.hash(bitcoinMergedMiningCoinbaseTransactionOneRoundOfHash));

        if (!mpValidator.isValid(
                bitcoinMergedMiningBlock.getMerkleRoot(),
    ...
    private static boolean validFallbackBlockSignature(
        ...
        ECKey pub =
                ECKey.recoverFromSignature(
                        signature.v - 27, signature, header.getHashForMergedMining(), false);
