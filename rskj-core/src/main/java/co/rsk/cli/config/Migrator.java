    ...
    public String migrateConfiguration() throws IOException {
        return migrateConfiguration(
                Files.newBufferedReader(
                        configuration.getSourceConfiguration(), StandardCharsets.UTF_8),
                configuration.getMigrationConfiguration());
    }

    public static String migrateConfiguration(
            Reader sourceReader, Properties migrationConfiguration) {
            ...
            if (originalPath.startsWith(NEW_CONFIG_PREFIX)) {
                try {
                    String newConfigPath =
                            originalPath.substring(NEW_CONFIG_PREFIX.length()).trim();
                    ...
                    throw new IllegalArgumentException(
                            String.format(
                                    "Unable to parse value for the %s property", originalPath),
                            e);
                }
