...
/**
 * MinerClient mines new blocks. In fact it just performs the proof-of-work needed to find a valid block and uses uses
 * MinerServer to build blocks to mine and publish blocks once a valid nonce was found.
 *
...
public class MinerClientImpl implements MinerClient {
    ...
    public MinerClientImpl(
            Rsk rsk, MinerServer minerServer, Duration delayBetweenBlocks, Duration delayBetweenRefreshes) {
        this.rsk = rsk;
    ...
    public boolean mineBlock() {
        ...
        co.rsk.bitcoinj.core.BtcTransaction bitcoinMergedMiningCoinbaseTransaction =
                MinerUtils.getBitcoinMergedMiningCoinbaseTransaction(bitcoinNetworkParameters, work);
    ...
    /**
     * findNonce will try to find a valid nonce for bitcoinMergedMiningBlock, that satisfies the given target
     * difficulty.
     *
     ...
     * @remarks This method will return if the stop or newBetBlockArrivedFromAnotherNode intance variables are set to
     *     true.
     */
    private boolean findNonce(
            @Nonnull final co.rsk.bitcoinj.core.BtcBlock bitcoinMergedMiningBlock, @Nonnull final BigInteger target) {
