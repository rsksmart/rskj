...
public class Rskip92MerkleProofBuilder implements MerkleProofBuilder {
    ...
    public byte[] buildFromMerkleHashes(
        ...
        Stream<byte[]> hashesStream =
                merkleHashesString.stream().map(mh -> Utils.reverseBytes(Hex.decode(mh)));
    ...
    public byte[] buildFromTxHashes(BtcBlock blockWithHeaderOnly, List<String> txHashesString) {
        return buildFromFullPmt(
                new GenesisMerkleProofBuilder()
                        .buildFromTxHashes(blockWithHeaderOnly, txHashesString));
    }
    ...
    /**
     * This takes a full PMT and slices and rearranges the needed pieces for the new serialization
     * format. It would make sense to re-implement this as a standalone algorithm, but reusing the
     * PMT is enough for now.
     */
    private byte[] buildFromFullPmt(byte[] pmtSerialized) {
        Stream<byte[]> hashesStream =
                PartialMerkleTreeFormatUtils.streamIntermediateHashes(pmtSerialized)
