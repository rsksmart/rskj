...
public class Trie {
    ...
    private Trie internalPut(TrieKeySlice key, byte[] value, boolean isRecursiveDelete) {
        ...
        if (sharedPath.length() >= key.length()) {
            ...
            return new Trie(this.sharedPath, value, this.left, this.right, this.store, getDataLength(value), null, this.isSecure);
        }

        if (isEmptyTrie()) {
            return new Trie(this.store, key, value, this.isSecure, getDataLength(value), null);
        }
