...
public class TrieConverter {
                ...
                cacheHashes.computeIfAbsent(
                        ...
                        k -> {
                            ...
                            return trie == null
                                    ...
                                    : trie.getHashOrchid(true).getBytes();
                        });
    ...
    private Trie getOrchidAccountTrieRoot(TrieKeySlice key, Trie src, boolean removeFirst8bits) {
        ...
        boolean isRemascAccount =
                key.length()
                        == (1
                                        + TrieKeyMapper.SECURE_KEY_SIZE
                                        + RemascTransaction.REMASC_ADDRESS.getBytes().length)
                                * Byte.SIZE;
        if ((key.length()
            ...
            OrchidAccountState oldState =
                    new OrchidAccountState(astate.getNonce(), astate.getBalance());
            ...
            if (child0 != null) {
                ...
                TrieKeySlice child0Key =
                        key.rebuildSharedPath(LEFT_CHILD_IMPLICIT_KEY, child0.getSharedPath());
            ...
            if (isRemascAccount) {
                orchidKey =
                        extractOrchidAccountKeyPathFromUnitrieKey(
                                key,
                                sharedPath.length(),
                                RemascTransaction.REMASC_ADDRESS.getBytes().length,
                                TrieKeyMapper.REMASC_ACCOUNT_KEY_SIZE);
            } else {
                orchidKey =
                        extractOrchidAccountKeyPathFromUnitrieKey(
                                key,
                                sharedPath.length(),
                                RskAddress.LENGTH_IN_BYTES,
        ...
        if (child1 != null) {
            TrieKeySlice child1Key =
                    key.rebuildSharedPath(RIGHT_CHILD_IMPLICIT_KEY, child1.getSharedPath());
    ...
    private Trie getOrchidStateRoot(
        ...
        Trie child0 =
                unitrieStorageRoot.getNodeReference(LEFT_CHILD_IMPLICIT_KEY).getNode().orElse(null);
        Trie child1 =
                unitrieStorageRoot
                        .getNodeReference(RIGHT_CHILD_IMPLICIT_KEY)
                        .getNode()
                        .orElse(null);
        Trie child0Hash = null;
        if (child0 != null) {
            ...
            child0Hash =
                    getOrchidStateRoot(
                            child0Key, child0, false, removeFirstNodePrefix && child1 == null);
        }
        ...
        if (child1 != null) {
            ...
            child1Hash =
                    getOrchidStateRoot(
                            child1Key, child1, false, removeFirstNodePrefix && child0 == null);
        }
        ...
        if (!removeFirstNodePrefix && child0Hash == null && child1Hash == null) { // terminal node
            ...
            // 42 = DOMAIN_PREFIX(1) + SECURE_KEY_SIZE(10) + RskAddress(20) + STORAGE_PREFIX(1) +
            // SECURE_KEY_SIZE(10)
            if (key.length() >= 42 * Byte.SIZE) {
                // there is a branching ahead of the needed shared 10 bytes
                throw new IllegalArgumentException(
                        "The unitrie storage doesn't share as much structure as we need to rebuild the Orchid trie");
            }
        }

        NodeReference left = new NodeReference(null, child0Hash, null);
        NodeReference right = new NodeReference(null, child1Hash, null);
        Trie newNode = new Trie(null, sharedPath, value, left, right, valueLength, valueHash);
        if (!onlyChild) {
    ...
    private TrieKeySlice extractOrchidAccountKeyPathFromUnitrieKey(
            TrieKeySlice key,
            int sharedPathLength,
            int addressLengthInBytes,
            int unitrieKeySizeInBytes) {
        if (sharedPathLength
                < (unitrieKeySizeInBytes - TrieKeyMapper.SECURE_KEY_SIZE)
                        * Byte.SIZE) { // = 20 bytes = RskAddress.LENGTH_IN_BYTES
            throw new IllegalArgumentException(
                    "The unitrie doesn't share as much structure as we need to rebuild the Orchid trie");
        }

        byte[] encodedKey =
                key.slice(key.length() - addressLengthInBytes * Byte.SIZE, key.length()).encode();
        ...
        return expandedOrchidTrieSecureKey.slice(
                commonTriePathLength, expandedOrchidTrieSecureKey.length());
    }

    private TrieKeySlice extractOrchidStorageKeyPathFromUnitrieKey(
            TrieKeySlice key, TrieKeySlice sharedPath) {
        // 42 = DOMAIN_PREFIX(1) + SECURE_KEY_SIZE(10) + RskAddress(20) + STORAGE_PREFIX(1) +
        // SECURE_KEY_SIZE(10)
        int leadingZeroesToAdd = 42 * Byte.SIZE + DataWord.BYTES * Byte.SIZE - key.length();
        TrieKeySlice unsecuredKey =
                key.slice(42 * Byte.SIZE, key.length()).leftPad(leadingZeroesToAdd);
        ...
        return expandedOrchidTrieSecureKey.slice(
                nonSharedPathOffset, expandedOrchidTrieSecureKey.length());
    }
}
