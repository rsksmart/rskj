...
/**
 * The Remasc contract which manages the distribution of miner fees. This class just extends
 * PrecompiledContract, checks no wrong data was supplied and delegates to Remasc the actual fee
 * distribution logic
 *
...
public class RemascContract extends PrecompiledContracts.PrecompiledContract {
    ...
    private static final CallTransaction.Function PROCESS_MINERS_FEES =
            CallTransaction.Function.fromSignature(
                    "processMinersFees", new String[] {}, new String[] {});
    ...
    public RemascContract(
        ...
        this.functions.put(
                new ByteArrayWrapper(PROCESS_MINERS_FEES.encodeSignature()), PROCESS_MINERS_FEES);
        this.functions.put(
                new ByteArrayWrapper(GET_STATE_FOR_DEBUGGING.encodeSignature()),
    ...
    public byte[] execute(byte[] data) {
        try {
            ...
            // Remasc contract was invoked with invalid parameters / out of place, throw
            // OutOfGasException to avoid funds being transferred
    ...
    private CallTransaction.Function getFunction(byte[] data) {
        ...
        if (data == null || data.length == 0) {
            ...
            if (data.length != 4) {
                logger.warn(
                        "Invalid function: signature longer than expected {}.",
