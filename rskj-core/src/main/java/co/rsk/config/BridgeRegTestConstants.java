...
public class BridgeRegTestConstants extends BridgeConstants {
    ...
    private static BridgeRegTestConstants instance =
            new BridgeRegTestConstants(REGTEST_FEDERATION_PUBLIC_KEYS);
    ...
    public BridgeRegTestConstants(List<BtcECKey> federationPublicKeys) {
        ...
        List<FederationMember> federationMembers =
                FederationMember.getFederationMembersFromKeys(federationPublicKeys);

        Instant genesisFederationCreatedAt =
                ZonedDateTime.parse("2016-01-01T00:00:00Z").toInstant();

        genesisFederation =
                new Federation(federationMembers, genesisFederationCreatedAt, 1L, getBtcParams());
