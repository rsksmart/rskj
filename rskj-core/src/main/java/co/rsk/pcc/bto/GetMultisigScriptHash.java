...
/**
 * This implements the "getMultisigScriptHash" method that belongs to the HDWalletUtils native
 * contract.
 *
...
public class GetMultisigScriptHash extends NativeMethod {
    ...
    // (see
    // https://github.com/bitcoin/bips/blob/master/bip-0016.mediawiki#520byte_limitation_on_serialized_script_size)
    ...
    public Object execute(Object[] arguments) {
        ...
        Arrays.stream(publicKeys)
                .forEach(
                        o -> {
                            ...
                            // Avoid extra work by not recalculating compressed keys on already
                            // compressed keys
                            try {
                                ...
                                if (publicKey.length == UNCOMPRESSED_PUBLIC_KEY_LENGTH) {
                                    btcPublicKey =
                                            BtcECKey.fromPublicOnly(
                                                    btcPublicKey.getPubKeyPoint().getEncoded(true));
                                }
                                btcPublicKeys.add(btcPublicKey);
                            } catch (IllegalArgumentException e) {
                                throw new NativeContractIllegalArgumentException(
                                        String.format(
                                                "Invalid public key format: %s",
        ...
        Script multisigScript =
                ScriptBuilder.createP2SHOutputScript(minimumSignatures, btcPublicKeys);
