...
public class MissingOrchidStorageKeysProvider {
    ...
    private static final Logger logger =
            LoggerFactory.getLogger(MissingOrchidStorageKeysProvider.class);
    ...
    public DataWord getKeccak256PreImage(Keccak256 storageKeyHash) {
        if (patchDatabase == null) {
            if (!databaseLocalFile.exists()) {
                try (ReadableByteChannel patchDatabaseStream =
                                Channels.newChannel(missingStorageKeysRemoteUrl.openStream());
                        FileOutputStream fileOutputStream =
                                new FileOutputStream(databaseLocalFile)) {
                    fileOutputStream
                            .getChannel()
                            .transferFrom(patchDatabaseStream, 0, Long.MAX_VALUE);
            ...
            patchDatabase =
                    indexDB.hashMapCreate("preimages")
        ...
        if (!Arrays.equals(storageKeyHash.getBytes(), Keccak256Helper.keccak256(storageKey))) {
            throw new IllegalStateException(
                    String.format(
                            "You have downloaded an inconsistent database. %s doesn't match expected keccak256 hash (%s)",
