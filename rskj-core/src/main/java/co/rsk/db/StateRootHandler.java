...
public class StateRootHandler {
    ...
    public void register(BlockHeader executedBlock, Trie executionResult) {
        ...
        if (executedBlock.isGenesis()) {
            Keccak256 orchidStateRoot = new Keccak256(trieConverter.getOrchidAccountTrieRoot(executionResult));
            stateRootTranslator.put(blockStateRoot, orchidStateRoot); // workaround for BlockChainLoader.updateGenesis
