...
/**
 * Precompiled contract that manages the 2 way peg between bitcoin and RSK. This class is just a wrapper, actual
 * functionality is found in BridgeSupport.
 *
...
public class Bridge extends PrecompiledContracts.PrecompiledContract {
    ...
    // Returns an array of block hashes known by the bridge contract. Federators can use this to find what is the latest
    // block in the mainchain the bridge has.
    ...
    // Returns the confirmations number of the block for the given transaction. If its not valid or its not part of the
    // main chain it returns a negative number
    ...
    public void receiveHeaders(Object[] args) {
        ...
        if (Arrays.stream(btcBlockSerializedArray)
                .anyMatch(
                        bytes -> !BtcTransactionFormatUtils.isBlockHeaderSize(((byte[]) bytes).length, activations))) {
        ...
        for (int i = 0; i < btcBlockSerializedArray.length; i++) {
            ...
            try {
                ...
                throw new BridgeIllegalArgumentException(
                        "Block " + i + " could not be parsed " + Hex.toHexString(btcBlockSerialized), e);
            }
        }
        try {
    ...
    public void addSignature(Object[] args) {
        ...
        try {
            ...
            throw new BridgeIllegalArgumentException(
                    "Public key could not be parsed " + Hex.toHexString(federatorPublicKeySerialized), e);
        }
    ...
    public Integer createFederation(Object[] args) {
        ...
        return bridgeSupport.voteFederationChange(rskTx, new ABICallSpec("create", new byte[][] {}));
    }
    ...
    public Integer addFederatorPublicKey(Object[] args) {
        ...
        return bridgeSupport.voteFederationChange(rskTx, new ABICallSpec("add", new byte[][] {publicKeyBytes}));
    }
    ...
    public Integer commitFederation(Object[] args) {
        ...
        return bridgeSupport.voteFederationChange(rskTx, new ABICallSpec("commit", new byte[][] {hash}));
    }

    public Integer rollbackFederation(Object[] args) {
        logger.trace("rollbackFederation");

        return bridgeSupport.voteFederationChange(rskTx, new ABICallSpec("rollback", new byte[][] {}));
    }
        ...
        return (self, args) -> {
            ...
            if (!BridgeUtils.isFromFederateMember(self.rskTx, self.bridgeSupport.getActiveFederation())
                    && (retiringFederation == null
                            || (retiringFederation != null
                                    && !BridgeUtils.isFromFederateMember(self.rskTx, retiringFederation)))) {
                String errorMessage =
                        String.format(
                                "Sender is not part of the active or retiring federations, so he is not enabled to"
                                    + " call the function '%s'",
                                funcName);
