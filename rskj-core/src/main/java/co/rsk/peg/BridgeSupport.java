...
public class BridgeSupport {
    ...
    private static final List<String> FEDERATION_CHANGE_FUNCTIONS =
            Collections.unmodifiableList(Arrays.asList("create", "add", "add-multi", "commit", "rollback"));
    ...
    public BridgeSupport(
        ...
        this(
                bridgeConstants,
                new BridgeStorageProvider(repository, contractAddress, bridgeConstants, bridgeStorageConfiguration),
                eventLogger,
                repository,
                rskExecutionBlock,
    ...
    InputStream getCheckPoints() {
        InputStream checkpoints =
                BridgeSupport.class.getResourceAsStream(
                        "/rskbitcoincheckpoints/" + bridgeConstants.getBtcParams().getId() + ".checkpoints");
        if (checkpoints == null) {
    ...
    /**
     * Get the wallet for the currently retiring federation or null if there's currently no retiring federation
     *
     * @return A BTC wallet for the currently active federation
     * @throws IOException
     */
    ...
    /**
     * In case of a lock tx: Transfers some SBTCs to the sender of the btc tx and keeps track of the new UTXOs available
     * for spending. In case of a release tx: Keeps track of the change UTXOs, now available for spending.
     *
    ...
    public void registerBtcTransaction(Transaction rskTx, byte[] btcTxSerialized, int height, byte[] pmtSerialized)
        ...
        if (height < 0) {
            String panicMessage =
                    String.format("Btc Tx %s Supplied Height is %d but should be greater than 0", btcTxHash, height);
        ...
        try {
            ...
            throw new BridgeIllegalArgumentException(
                    String.format("PartialMerkleTree could not be parsed {}", Hex.toHexString(pmtSerialized)), e);
        }
        ...
        if (BridgeUtils.isLockTx(btcTx, getLiveFederations(), btcContext, bridgeConstants)) {
            ...
            if (!lockWhitelist.isWhitelistedFor(senderBtcAddress, totalAmount, height)) {
                ...
                if (buildReturnResult.isPresent()) {
                    ...
                    panicProcessor.panic(
                            ...
                            String.format(
                                    "whitelist money return tx build for btc tx %s error. Return was to %s. Tx %s."
                                        + " Value %s",
                ...
                rskRepository.transfer(
                        PrecompiledContracts.BRIDGE_ADDR, sender, co.rsk.core.Coin.fromBitcoin(totalAmount));
            ...
            // a) In testnet, devnet or local: we restart the RSK blockchain whithout changing the federation address.
            // We don't want to have utxos that were already spent.
            // Open problem: TxA spends TxB. registerBtcTransaction() for TxB is called, it spends a utxo the bridge is
            // not yet aware of,
            // so nothing is removed. Then registerBtcTransaction() for TxA and the "already spent" utxo is added as it
            // was not spent.
            // When is not guaranteed to be called in the chronological order, so a Federator can inform
            // b) In prod: Federator created a tx manually or the federation was compromised and some utxos were spent.
            // Better not try to spend them.
    ...
    */
    ...
    /**
     * Initiates the process of sending coins back to BTC. This is the default contract method. The funds will be sent
     * to the bitcoin address controlled by the private key that signed the rsk tx. The amount sent to the bridge in
     * this tx will be the amount sent in the btc network minus fees.
     *
     ...
     * @return true if the request was successfully added, false if the value to release was considered dust and
     *     therefore ignored.
     * @throws IOException
     */
    ...
    /**
     * Executed every now and then. Performs a few tasks: processing of any pending btc funds migrations from retiring
     * federations; processing of any outstanding btc release requests; and processing of any outstanding release btc
     * transactions.
     *
    ...
    private boolean federationIsInMigrationAge(Federation federation) {
        ...
        long ageEnd =
                bridgeConstants.getFederationActivationAge() + bridgeConstants.getFundsMigrationAgeSinceActivationEnd();
    ...
    private void processFundsMigration() throws IOException {
        ...
        if (federationIsInMigrationAge(activeFederation) && hasMinimumFundsToMigrate(retiringFederationWallet)) {
            ...
            Pair<BtcTransaction, List<UTXO>> createResult =
                    createMigrationTransaction(retiringFederationWallet, activeFederation.getAddress());
    ...
    /**
     * Processes the current btc release request queue and tries to build btc transactions using (and marking as spent)
     * the current active federation's utxos. Newly created btc transactions are added to the btc release tx set, and
     * failed attempts are kept in the release queue for future processing.
     */
    private void processReleaseRequests() {
        ...
        final ReleaseTransactionBuilder txBuilder =
                new ReleaseTransactionBuilder(
                        btcContext.getParams(), activeFederationWallet, getFederationAddress(), getFeePerKb());
        ...
        releaseRequestQueue.process(
                ...
                (ReleaseRequestQueue.Entry releaseRequest) -> {
                    Optional<ReleaseTransactionBuilder.BuildResult> result =
                            txBuilder.buildAmountTo(releaseRequest.getDestination(), releaseRequest.getAmount());
                    ...
                    try {
                        ...
                        logger.error(
                                String.format(
                                        "Unexpected error accessing storage while attempting to add a release BTC tx"
                                            + " for <%s, %s>",
    ...
    /**
     * Processes the current btc release transaction set. It basically looks for transactions with enough confirmations
     * and marks them as ready for signing as well as removes them from the set.
     *
    ...
    /**
     * If federation change output value had to be increased to be non-dust, the federation now has more BTC than it
     * should. So, we burn some sBTC to make balances match.
     *
    ...
    private void adjustBalancesIfChangeOutputWasDust(BtcTransaction btcTx, Coin sentByUser) {
        ...
        if (spentByFederation.isLessThan(sentByUser)) {
            ...
            rskRepository.transfer(
                    PrecompiledContracts.BRIDGE_ADDR, BURN_ADDRESS, co.rsk.core.Coin.fromBitcoin(coinsToBurn));
        }
    }

    /**
     * Adds a federator signature to a btc release tx. The hash for the signature must be calculated with
     * Transaction.SigHash.ALL and anyoneCanPay=false. The signature must be canonical. If enough signatures were added,
     * ask federators to broadcast the btc release tx.
     *
    ...
    public void addSignature(BtcECKey federatorPublicKey, List<byte[]> signatures, byte[] rskTxHash) throws Exception {
        ...
        if (!getActiveFederation().getBtcPublicKeys().contains(federatorPublicKey)
            ...
            logger.warn(
                    "Supplied federator public key {} does not belong to any of the federators.", federatorPublicKey);
            return;
        }
    ...
    private void processSigning(
            BtcECKey federatorPublicKey, List<byte[]> signatures, byte[] rskTxHash, BtcTransaction btcTx)
            throws IOException {
    ...
    /**
     * @deprecated Returns an array of block hashes known by the bridge contract. Federators can use this to find what
     *     is the latest block in the mainchain the bridge has.
    ...
    public Integer getBtcTransactionConfirmations(
        ...
        try {
            ...
            logger.warn(String.format("Illegal state trying to get block with hash {}", btcBlockHash), e);
     ...
     * @return a Boolean indicating whether the given btc tx hash was already processed by the bridge.
     * @throws IOException
     */
    ...
    /**
     * Returns the RSK blockchain height a given btc transaction hash was processed at by the bridge.
     *
    ...
    public byte[] getRetiringFederatorPublicKey(int index) {
        ...
        if (index < 0 || index >= publicKeys.size()) {
            throw new IndexOutOfBoundsException(
                    String.format("Retiring federator index must be between 0 and %d", publicKeys.size() - 1));
        }
    ...
    /**
     * Returns the currently live federations This would be the active federation plus potentially the retiring
     * federation
     *
    ...
    /**
     * Creates a new pending federation If there's currently no pending federation and no funds remain to be moved from
     * a previous federation, a new one is created. Otherwise, -1 is returned if there's already a pending federation,
     * -2 is returned if there is a federation awaiting to be active, or -3 if funds are left from a previous one.
     *
     * @param dryRun whether to just do a dry run
     * @return 1 upon success, -1 when a pending federation is present, -2 when a federation is to be activated, and if
     *     -3 funds are still to be moved between federations.
     */
     ...
     * @return 1 upon success, -1 if there was no pending federation, -2 if the key was already in the pending
     *     federation
     */
    ...
    /**
     * Commits the currently pending federation. That is, the retiring federation is set to be the currently active
     * federation, the active federation is replaced with a new federation generated from the pending federation, and
     * the pending federation is wiped out. Also, UTXOs are moved from active to retiring so that the transfer of funds
     * can begin.
     *
     * @param dryRun whether to just do a dry run
     * @param hash the pending federation's hash. This is checked the execution block's pending federation hash for
     *     equality.
     * @return 1 upon success, -1 if there was no pending federation, -2 if the pending federation was incomplete, -3 if
     *     the given hash doesn't match the current pending federation's hash.
     */
    private Integer commitFederation(boolean dryRun, Keccak256 hash) throws IOException {
        ...
        provider.setNewFederation(
                currentPendingFederation.buildFederation(
                        creationTime, rskExecutionBlock.getNumber(), bridgeConstants.getBtcParams()));
    ...
    private ABICallVoteResult executeVoteFederationChangeFunction(boolean dryRun, ABICallSpec callSpec)
            throws IOException {
        // Try to do a dry-run and only register the vote if the
        // call would be successful
        ABICallVoteResult result;
        ...
        switch (callSpec.getFunction()) {
            ...
            case "add-multi":
                ...
                try {
                    ...
                    throw new BridgeIllegalArgumentException(
                            "BTC public key could not be parsed " + Hex.toHexString(callSpec.getArguments()[0]), e);
                }
                ...
                try {
                    ...
                    throw new BridgeIllegalArgumentException(
                            "RSK public key could not be parsed " + Hex.toHexString(callSpec.getArguments()[1]), e);
                }
                ...
                try {
                    ...
                    throw new BridgeIllegalArgumentException(
                            "MST public key could not be parsed " + Hex.toHexString(callSpec.getArguments()[2]), e);
                }
    ...
    public byte[] getPendingFederatorPublicKey(int index) {
        ...
        if (index < 0 || index >= publicKeys.size()) {
            throw new IndexOutOfBoundsException(
                    String.format("Federator index must be between 0 and %d", publicKeys.size() - 1));
        }
    ...
    /**
     * Returns the lock whitelist address stored at the given index, or null if the index is out of bounds
     *
    ...
    /**
     * Adds the given address to the lock whitelist. Returns 1 upon success, or -1 if the address was already in the
     * whitelist.
     *
     ...
     * @return 1 upon success, -1 if the address was already in the whitelist, -2 if address is invalid
    ...
    /**
     * Removes the given address from the lock whitelist. Returns 1 upon success, or -1 if the address was not in the
     * whitelist.
     *
     * @param addressBase58 the base58-encoded address to remove from the whitelist
     * @return 1 upon success, -1 if the address was not in the whitelist, -2 if the address is invalid,
     *     LOCK_WHITELIST_GENERIC_ERROR_CODE otherwise.
     */
     ...
     *
     * @return 1 upon successful vote, -1 when the vote was unsuccessful, FEE_PER_KB_GENERIC_ERROR_CODE when there was
    ...
    public Integer voteFeePerKbChange(Transaction tx, Coin feePerKb) {
        ...
        ABICallSpec feeVote =
                new ABICallSpec("setFeePerKb", new byte[][] {BridgeSerializationUtils.serializeCoin(feePerKb)});
