...
/**
 * Historic record of which pegout {@code getNextPegoutWithEnoughConfirmations} selected when more
 * than one entry was eligible at the same call, keyed by the confirming {@code updateCollections}
 * rsk tx hash.
 *
 ...
 * Java 17 and Java 21. This lookup lets a node reproduce the exact historic selection on any JVM,
 * without depending on HashSet iteration order. After RSKIP559 activates, selection is deterministic
 * via {@code Entry.BTC_TX_COMPARATOR} sorting and this table is no longer consulted.</p>
 *
 * <ul>
 *   <li><b>key</b>   = the {@code updateCollections} rsk tx hash that made the selection
 *                      (lowercase hex, no {@code 0x} prefix)</li>
 *   <li><b>value</b> = the selected entry's btc tx hash
 *                      (as {@code Sha256Hash.toString()}, lowercase hex, no {@code 0x} prefix)</li>
 * </ul>
 *
...
public final class HistoricalPegoutSelections {
    ...
    private static final Map<String, String> MAINNET_SELECTIONS;
    private static final Map<String, String> TESTNET_SELECTIONS;
    ...
    static {
        ...
        MAINNET_SELECTIONS = Collections.unmodifiableMap(mainnet);
        ...
        TESTNET_SELECTIONS = Collections.unmodifiableMap(testnet);
    }

    private HistoricalPegoutSelections() {
    }

    /**
     * Whether a hardcoded historical selection dataset exists for the given network. Only mainnet and
    ...
    /**
     * The btc tx hash of the pegout historically selected by the {@code updateCollections} identified
     * by {@code rskTxHash}, or empty if that call is not in the dataset (i.e. it did not have more
     * than one eligible entry).
     *
     * @param rskTxHash the confirming updateCollections rsk tx hash (with or without {@code 0x}).
    ...
    private static Map<String, String> selectMap(BridgeConstants bridgeConstants) {
        String networkId = bridgeConstants.getBtcParams().getId();
        if (NetworkParameters.ID_MAINNET.equals(networkId)) {
            return MAINNET_SELECTIONS;
        }
        if (NetworkParameters.ID_TESTNET.equals(networkId)) {
            return TESTNET_SELECTIONS;
        }
        throw new IllegalStateException("No historical pegout selections are defined for network: " + networkId);
    }
