...
public class BridgeSerializationUtils {
    ...
    public static Map<Sha256Hash, Long> deserializeMapOfHashesToLong(byte[] data) {
        ...
        for (int k = 0; k < numEntries; k++) {
            ...
            Long number =
                    BigIntegers.fromUnsignedByteArray(rlpList.get(k * 2 + 1).getRLPData())
                            .longValue();
            map.put(hash, number);
        }
    ...
    /**
     * A federation is serialized as a list in the following order: - creation time - creation block
     * number - list of federation members -> [member1, member2, ..., membern], sorted using the
     * lexicographical order of the public keys of the members (see
     * FederationMember.BTC_RSK_MST_PUBKEYS_COMPARATOR). Each federation member is in turn
    ...
    private static byte[] serializeFederationWithSerializer(
        ...
        List<byte[]> federationMembers =
                federation.getMembers().stream()
                        ...
                        .map(
                                member ->
                                        RLP.encodeElement(
                                                federationMemberSerializer.serialize(member)))
                        .collect(Collectors.toList());

        byte[][] rlpElements = new byte[FEDERATION_RLP_LIST_SIZE][];
        rlpElements[FEDERATION_CREATION_TIME_INDEX] =
                RLP.encodeBigInteger(
                        BigInteger.valueOf(federation.getCreationTime().toEpochMilli()));
        ...
        rlpElements[FEDERATION_MEMBERS_INDEX] =
                RLP.encodeList(
                        (byte[][]) federationMembers.toArray(new byte[federationMembers.size()][]));
    ...
    private static Federation deserializeFederationWithDesserializer(
        ...
        Instant creationTime =
                Instant.ofEpochMilli(
                        BigIntegers.fromUnsignedByteArray(creationTimeBytes).longValue());

        byte[] creationBlockNumberBytes =
                rlpList.get(FEDERATION_CREATION_BLOCK_NUMBER_INDEX).getRLPData();
        long creationBlockNumber =
                BigIntegers.fromUnsignedByteArray(creationBlockNumberBytes).longValue();
        ...
        List<FederationMember> federationMembers =
                ((RLPList) rlpList.get(FEDERATION_MEMBERS_INDEX))
                        .stream()
                                .map(
                                        memberBytes ->
                                                federationMemberDesserializer.deserialize(
                                                        memberBytes.getRLPData()))
                                .collect(Collectors.toList());

        return new Federation(
                federationMembers, creationTime, creationBlockNumber, networkParameters);
    }

    /**
     * For the federation serialization format, see serializeFederationWithSerializer. For
     * compatibility with blocks before the Wasabi network upgrade, each federation member is
     * serialized only as its compressed BTC public key.
     */
    ...
    public static Federation deserializeFederationOnlyBtcKeys(
        ...
        return deserializeFederationWithDesserializer(
                ...
                (pubKeyBytes ->
                        FederationMember.getFederationMemberFromKey(
                                BtcECKey.fromPublicOnly(pubKeyBytes))));
    }

    /**
     * For the federation serialization format, see serializeFederationWithSerializer. For the
     * federation member serialization format, see serializeFederationMember.
     */
    ...
    /**
     * A FederationMember is serialized as a list in the following order: - BTC public key - RSK
     * public key - MST public key All keys are stored in their COMPRESSED versions.
     */
    ...
    private static FederationMember deserializeFederationMember(byte[] data) {
        ...
        BtcECKey btcKey =
                BtcECKey.fromPublicOnly(rlpList.get(FEDERATION_MEMBER_BTC_KEY_INDEX).getRLPData());
        ECKey rskKey =
                ECKey.fromPublicOnly(rlpList.get(FEDERATION_MEMBER_RSK_KEY_INDEX).getRLPData());
        ECKey mstKey =
                ECKey.fromPublicOnly(rlpList.get(FEDERATION_MEMBER_MST_KEY_INDEX).getRLPData());

        return new FederationMember(btcKey, rskKey, mstKey);
    }

    /**
     * A pending federation is serialized as the public keys conforming it. This is a legacy format
     * for blocks before the Wasabi network upgrade. See
     * BridgeSerializationUtils::serializeBtcPublicKeys
     */
    public static byte[] serializePendingFederationOnlyBtcKeys(
            PendingFederation pendingFederation) {
        return serializeBtcPublicKeys(pendingFederation.getBtcPublicKeys());
    }

    // For the serialization format, see
    ...
    /**
     * A pending federation is serialized as the list of its sorted members serialized. For the
     * member serialization format, see BridgeSerializationUtils::serializeFederationMember
     */
    ...
    public static ABICallElection deserializeElection(
            byte[] data, AddressBasedAuthorizer authorizer) {
        if (data == null || data.length == 0) {
        ...
        if (rlpList.size() % 2 != 0) {
            throw new RuntimeException(
                    "deserializeElection: expected an even number of entries, but odd given");
        }
     ...
     *
     * @param data data MUST be composed of a list of {@link
     *     co.rsk.peg.whitelist.OneOffWhiteListEntry} and the value of disableBlockHeight obtained
     *     from {@link co.rsk.peg.whitelist.LockWhitelist}
     * @return the serialized data
     */
    public static byte[] serializeOneOffLockWhitelist(
        ...
        serializedLockWhitelist[serializationSize - 1] =
                RLP.encodeBigInteger(BigInteger.valueOf(disableBlockHeight));
    ...
    public static Pair<HashMap<Address, OneOffWhiteListEntry>, Integer>
        ...
        if (serializedAddressesSize % 2 != 0) {
            throw new RuntimeException(
                    "deserializeLockWhitelist: expected an even number of addresses, but odd given");
        }
        ...
        for (int i = 0; i < serializedAddressesSize; i = i + 2) {
            ...
            entries.put(
                    ...
                    new OneOffWhiteListEntry(
                            address,
                            Coin.valueOf(safeToBigInteger(maxTransferValueData).longValueExact())));
        }
        int disableBlockHeight =
                safeToBigInteger(rlpList.get(serializedAddressesSize).getRLPData()).intValueExact();
    ...
    public static Map<Address, UnlimitedWhiteListEntry> deserializeUnlimitedLockWhitelistEntries(
        ...
        int unlimitedWhitelistEntriesSerializedAddressesSize =
                unlimitedWhitelistEntriesRlpList.size();

        Map<Address, UnlimitedWhiteListEntry> entries =
                new HashMap<>(unlimitedWhitelistEntriesSerializedAddressesSize);
    ...
    public static ReleaseRequestQueue deserializeReleaseRequestQueue(
            byte[] data, NetworkParameters networkParameters) {
    ...
    public static byte[] serializeReleaseTransactionSet(ReleaseTransactionSet set) {
        List<ReleaseTransactionSet.Entry> entries =
                set.getEntries().stream().collect(Collectors.toList());
    ...
    private static byte[] serializeABICallSpec(ABICallSpec spec) {
        byte[][] encodedArguments =
                Arrays.stream(spec.getArguments())
    ...
    private static ABICallSpec deserializeABICallSpec(byte[] data) {
        ...
        if (rlpList.size() != 2) {
            throw new RuntimeException(
                    String.format(
                            "Invalid serialized ABICallSpec. Expected 2 elements, but got %d",
                            rlpList.size()));
        }

        String function = new String(rlpList.get(0).getRLPData(), StandardCharsets.UTF_8);
        byte[][] arguments =
                ((RLPList) rlpList.get(1))
                        .stream().map(rlpElement -> rlpElement.getRLPData()).toArray(byte[][]::new);
