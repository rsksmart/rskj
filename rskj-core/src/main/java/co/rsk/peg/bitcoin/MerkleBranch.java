...
/**
 * Represents a branch of a merkle tree. Can be used to validate that a certain transaction belongs
 * to a certain block.
 *
 * <p>IMPORTANT: this class can only be used to validate an existing merkle branch against a <block,
 * transaction> pair. It cannot be used to generate a merkle branch from a complete block and a
 * single transaction, since there's no use case for it.
 *
...
public class MerkleBranch {
    ...
    /**
     * Returns true if and only if this merkle branch successfully proves that tx hash is included
     * in block.
     *
    ...
    /**
     * Given a transaction hash, this method traverses the path, calculating the intermediate hashes
     * and ultimately arriving at the merkle root.
     *
     * @param txHash The transaction hash
