...
public class UtxoUtils {
     ...
     * @return {@code List<Coin>} the list of outpoint values decoded in the same ordered they were before encoding.
     * Or an {@code Collections.EMPTY_LIST} when {@code encodedOutpointValues} is {@code null} or {@code empty byte[]}.
     */
     ...
     * @return {@code byte[]} the list of outpoint values encoded preserving the order of the entries.
     * Or an {@code empty byte[]} when {@code outpointValues} is {@code null} or {@code empty}.
     */
    ...
    private static void assertValidOutpointValue(Coin outpointValue) {
        if (outpointValue == null || outpointValue.isNegative()){
            throw new InvalidOutpointValueException(String.format("Invalid outpoint value: %s. Negative and null values are not allowed.",
                outpointValue));
        }
    }
}
