...
public class BridgeUtils {
    ...
    public static Wallet getFederationsNoSpendWallet(
            Context btcContext, List<Federation> federations) {
    ...
    public static boolean isValidLockTx(BtcTransaction tx) {
        ...
        // This indicates that the tx is a P2PKH transaction which is the only one we support for
        // now
    ...
    /**
     * It checks if the tx doesn't spend any of the federations' funds and if it sends more than the
     * minimum ({@see BridgeConstants::getMinimumLockTxValue}) to any of the federations
     *
    ...
    public static boolean isLockTx(
        ...
        // First, check tx is not a typical release tx (tx spending from the any of the federation
        // addresses and
        ...
        for (int i = 0; i < tx.getInputs().size(); i++) {
            ...
            if (federations.stream()
                    .anyMatch(
                            federation ->
                                    scriptCorrectlySpendsTx(
                                            tx, index, federation.getP2SHScript()))) {
                return false;
            }
        }
        ...
        return (valueSentToMeSignum > 0
                && !valueSentToMe.isLessThan(bridgeConstants.getMinimumLockTxValue()));
    }
    ...
    public static boolean isReleaseTx(BtcTransaction tx, List<Federation> federations) {
        ...
        for (int i = 0; i < inputsSize; i++) {
            ...
            if (federations.stream()
                    ...
                    .anyMatch(
                            federationPayScript ->
                                    scriptCorrectlySpendsTx(tx, inputIndex, federationPayScript))) {
                return true;
            }
        }
        return false;
    ...
    public static boolean isContractTx(Transaction rskTx) {
        // TODO: this should be refactored to provide a more robust way of checking the transaction
        // origin
        return rskTx.getClass() == org.ethereum.vm.program.InternalTransaction.class;
    }

    public static boolean isFromFederateMember(
            org.ethereum.core.Transaction rskTx, Federation federation) {
