    ...
    public RedeemData findRedeemDataFromScriptHash(byte[] payToScriptHash) {
        ...
        Optional<Federation> destinationFederation =
                federations.stream()
                        .filter(
                                federation ->
                                        Arrays.equals(federation.getP2SHScript().getPubKeyHash(), payToScriptHash))
                        .findFirst();
        if (!destinationFederation.isPresent()) {
            return null;
        }
        return RedeemData.of(
                destinationFederation.get().getBtcPublicKeys(), destinationFederation.get().getRedeemScript());
    }
}
