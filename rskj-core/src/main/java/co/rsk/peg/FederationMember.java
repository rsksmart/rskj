 ...
 *
 * <p>It's composed of three public keys: one for the RSK network, one for the BTC network and one called MST of yet
 * undefined usage.
 *
...
public final class FederationMember {
     ...
     *
     * <p>The total ordering is defined such that, for any two members M1, M2, 1) M1 < M2 iff BTC_PUB_KEY(M1) @{literal
     * <}lex BTC_PUB_KEY(M2) OR (BTC_PUB_KEY(M1) ==lex BTC_PUB_KEY(M2) AND RSK_PUB_KEY(M1) @{literal <}lex
     * RSK_PUB_KEY(M2)) OR (BTC_PUB_KEY(M1) ==lex BTC_PUB_KEY(M2) AND RSK_PUB_KEY(M1) ==lex RSK_PUB_KEY(M2) AND
     * MST_PUB_KEY(M1) @{literal <}lex MST_PUB_KEY(M2)) 2) M1 == M2 iff BTC_PUB_KEY(M1) ==lex BTC_PUB_KEY(M2) AND
     * RSK_PUB_KEY(M1) ==lex RSK_PUB_KEY(M2) AND MST_PUB_KEY(M1) ==lex MST_PUB_KEY(M2) AND 3) M1 > M2 otherwise.
     *
            ...
            new Comparator<FederationMember>() {
                ...
                public int compare(FederationMember m1, FederationMember m2) {
                    int btcKeysComparison =
                            comparator.compare(m1.getBtcPublicKey().getPubKey(), m2.getBtcPublicKey().getPubKey());
                    if (btcKeysComparison == 0) {
                        int rskKeysComparison =
                                comparator.compare(m1.getRskPublicKey().getPubKey(), m2.getRskPublicKey().getPubKey());
                        if (rskKeysComparison == 0) {
                            return comparator.compare(
                                    m1.getMstPublicKey().getPubKey(), m2.getMstPublicKey().getPubKey());
                        }
