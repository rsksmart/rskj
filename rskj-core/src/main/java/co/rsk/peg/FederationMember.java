 ...
 *
 * <p>It's composed of three public keys: one for the RSK network, one for the BTC network and one
 * called MST of yet undefined usage.
 *
...
public final class FederationMember {
            ...
            switch (value) {
                ...
                default:
                    throw new IllegalArgumentException(
                            String.format("Invalid value for FederationMember.KeyType: %s", value));
            }
        }
    }

    // To be removed when different keys per federation member feature is implemented. These are
    // just helper
     ...
     * BTC_PUB_KEY(M1) <lex BTC_PUB_KEY(M2) OR (BTC_PUB_KEY(M1) ==lex BTC_PUB_KEY(M2) AND
     * RSK_PUB_KEY(M1) <lex RSK_PUB_KEY(M2)) OR (BTC_PUB_KEY(M1) ==lex BTC_PUB_KEY(M2) AND
     * RSK_PUB_KEY(M1) ==lex RSK_PUB_KEY(M2) AND MST_PUB_KEY(M1) <lex MST_PUB_KEY(M2)) 2) M1 == M2
     * iff BTC_PUB_KEY(M1) ==lex BTC_PUB_KEY(M2) AND RSK_PUB_KEY(M1) ==lex RSK_PUB_KEY(M2) AND
     * MST_PUB_KEY(M1) ==lex MST_PUB_KEY(M2) AND 3) M1 > M2 otherwise
     *
    ...
    public static final Comparator<FederationMember> BTC_RSK_MST_PUBKEYS_COMPARATOR =
            new Comparator<FederationMember>() {
                private Comparator<byte[]> comparator = UnsignedBytes.lexicographicalComparator();

                @Override
                public int compare(FederationMember m1, FederationMember m2) {
                    int btcKeysComparison =
                            comparator.compare(
                                    m1.getBtcPublicKey().getPubKey(),
                                    m2.getBtcPublicKey().getPubKey());
                    if (btcKeysComparison == 0) {
                        int rskKeysComparison =
                                comparator.compare(
                                        m1.getRskPublicKey().getPubKey(),
                                        m2.getRskPublicKey().getPubKey());
                        if (rskKeysComparison == 0) {
                            return comparator.compare(
                                    m1.getMstPublicKey().getPubKey(),
                                    m2.getMstPublicKey().getPubKey());
                        }
    ...
    public boolean equals(Object other) {
        ...
        return Arrays.equals(
                        btcPublicKey.getPubKey(), otherFederationMember.btcPublicKey.getPubKey())
                && Arrays.equals(
                        rskPublicKey.getPubKey(), otherFederationMember.rskPublicKey.getPubKey())
                && Arrays.equals(
                        mstPublicKey.getPubKey(), otherFederationMember.mstPublicKey.getPubKey());
    }
    ...
    public int hashCode() {
        ...
        return Objects.hash(btcPublicKey, rskPublicKey, mstPublicKey);
    }
}
