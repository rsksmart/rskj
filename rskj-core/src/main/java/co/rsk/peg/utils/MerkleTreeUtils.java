...
public class MerkleTreeUtils {
    ...
    private static long COINBASE_CHECKED =
            Utils.readUint32(new byte[] {(byte) 0xFF, (byte) 0xFF, (byte) 0xFF, (byte) 0xFF}, 0);

    private MerkleTreeUtils() {
    }

    /**
     * Combines two hashes (representing nodes in a merkle tree) to produce a single hash that would be the parent of
     * these two nodes.
     *
    ...
    public static Sha256Hash combineLeftRight(Sha256Hash left, Sha256Hash right) {
        ...
        return Sha256Hash.wrapReversed(
                Sha256Hash.hashTwice(reverseBytes(leftBytes), 0, 32, reverseBytes(rightBytes), 0, 32));
    }

    /**
     * Checks supplied bytes DO NOT represent a valid bitcoin transaction. Fixes attack described on
    ...
    private static void checkNotAValid64ByteTransaction(byte[] left, byte[] right) {
        ...
        // Check inputs count
        byte inputCount = leftAndRight[offset];
        offset += 1;
        if (inputCount != 1) {
        ...
        long output0Index = Utils.readUint32(leftAndRight, offset);
        offset += 4;
        ...
        byte input0ScriptLength = leftAndRight[offset];
        offset += 1;
        ...
        // Skip input 0 script
        offset += input0ScriptLength;

        // Skip input 0 sequence
        offset += 4;

        // Check output count
        byte outputCount = leftAndRight[offset];
        offset += 1;
        if (outputCount != 1) {
        ...
        long output0Value = Utils.readInt64(leftAndRight, offset);
        offset += 8;
        ...
        byte output0ScriptLength = leftAndRight[offset];
