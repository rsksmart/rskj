...
public class RLPElementView {
    ...
    private static int getLengthOfData(ByteBuffer data, byte lengthOfLength) {
        if (data.remaining() < 1 + lengthOfLength) {
            throw new RLPException(
                    "The length of the RLP item length can't possibly fit the data byte array");
        }
        ...
        if (Long.compareUnsigned(length, Integer.MAX_VALUE) > 0) {
            throw new RLPException(
                    "The current implementation doesn't support lengths longer than Integer.MAX_VALUE because that is the largest number of elements an array can have");
        }
