    ...
    public static RLPElementView calculateFirstElementInfo(@Nonnull ByteBuffer data) {
        try {
            ...
            if (prefix < OFFSET_SHORT_ITEM) {
                ...
                info.type = RLPElementType.ITEM;
                newData.position(1).limit(1 + prefix - OFFSET_SHORT_ITEM);
                ...
                newData.position(1 + lengthOfLength).limit(1 + lengthOfLength + lengthOfData);
            } else if (prefix < OFFSET_LONG_LIST) {
                // It's a list with a payload less than 55 bytes
                info.type = RLPElementType.SHORT_LIST;
                // because of how EthJ works, the RLPList has to have the full RLP data
                newData.position(1).limit(1 + prefix - OFFSET_SHORT_LIST);
            } else {
            ...
            throw new RLPException(
                    "The RLP byte array doesn't have enough space to hold an element with the specified length");
        }
    }
