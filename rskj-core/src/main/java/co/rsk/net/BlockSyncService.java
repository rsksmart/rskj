...
/**
 * BlockSyncService processes blocks to add into a blockchain. If a block is not ready to be added
...
public class BlockSyncService {
    ...
    private final BlockNodeInformation
            nodeInformation; // keep tabs on which nodes know which blocks.
    ...
    public BlockProcessResult processBlock(
            @Nonnull Block block, MessageChannel sender, boolean ignoreMissingHashes) {
        ...
        if (blockNumber > bestBlockNumber + syncMaxDistance) {
            logger.trace(
                    "Block too advanced {} {} from {} ",
                    blockNumber,
                    block.getShortHash(),
                    sender != null ? sender.getPeerNodeID().toString() : "N/A");
            return new BlockProcessResult(
                    false, null, block.getShortHash(), Duration.between(start, Instant.now()));
        }
        ...
        Set<Keccak256> unknownHashes =
                BlockUtils.unknownDirectAncestorsHashes(block, blockchain, store);
        // We can't add the block if there are missing ancestors or uncles. Request the missing
        // blocks to the sender.
        if (!unknownHashes.isEmpty()) {
            if (!ignoreMissingHashes) {
                logger.trace(
                        "Missing hashes for block in process {} {}",
                        blockNumber,
                        block.getShortHash());
        ...
        Map<Keccak256, ImportResult> connectResult =
                connectBlocksAndDescendants(
                        sender,
                        BlockUtils.sortBlocksByNumber(this.getParentsNotInBlockchain(block)),
                        ignoreMissingHashes);

        return new BlockProcessResult(
                true, connectResult, block.getShortHash(), Duration.between(start, Instant.now()));
    }
    ...
    private void tryReleaseStore(long bestBlockNumber) {
        if ((++processedBlocksCounter % PROCESSED_BLOCKS_TO_CHECK_STORE) == 0) {
            ...
            logger.trace(
                    "Blocks in block processor {} from height {} to height {}",
                    this.store.size(),
                    minimal,
                    maximum);
    ...
    private Map<Keccak256, ImportResult> connectBlocksAndDescendants(
            MessageChannel sender, List<Block> blocks, boolean ignoreMissingHashes) {
        Map<Keccak256, ImportResult> connectionsResult = new HashMap<>();
        List<Block> remainingBlocks = blocks;
        while (!remainingBlocks.isEmpty()) {
            Set<Block> connected =
                    getConnectedBlocks(
                            remainingBlocks, sender, connectionsResult, ignoreMissingHashes);
    ...
    private Set<Block> getConnectedBlocks(
            List<Block> remainingBlocks,
            MessageChannel sender,
            Map<Keccak256, ImportResult> connectionsResult,
            boolean ignoreMissingHashes) {
    ...
    /**
     * getParentsNotInBlockchain returns all the ancestors of the block (including the block itself)
     * that are not on the blockchain. It should be part of BlockChainImpl but is here because
     * BlockChain is coupled with the old org.ethereum.db.BlockStore.
     *
     * @param block the base block.
     * @return A list with the blocks sorted by ascending block number (the base block would be the
     *     last element).
     */
    @Nonnull
    private List<Block> getParentsNotInBlockchain(@Nullable Block block) {
        ...
        while (currentBlock != null
                && !blockchain.hasBlockInSomeBlockchain(currentBlock.getHash().getBytes())) {
    ...
    /**
     * getBlockFromStoreOrBlockchain retrieves a block from the store if it's available, or else
     * from the blockchain. It should be part of BlockChainImpl but is here because BlockChain is
     * coupled with the old org.ethereum.db.BlockStore.
     */
    @CheckForNull
