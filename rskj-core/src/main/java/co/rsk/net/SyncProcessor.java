...
public class SyncProcessor implements SyncEventsHandler {
    ...
    public SyncProcessor(
        ...
        this.syncInformation =
                new SyncInformationImpl(
                        blockHeaderValidationRule, blockValidationRule, difficultyCalculator);
        this.peerStatuses =
                new PeersInformation(syncInformation, channelManager, syncConfiguration);
        this.pendingMessages =
                new LinkedHashMap<Long, MessageType>() {
                    @Override
                    protected boolean removeEldestEntry(Map.Entry<Long, MessageType> eldest) {
                        boolean shouldDiscard = size() > MAX_PENDING_MESSAGES;
                        if (shouldDiscard) {
                            logger.trace(
                                    "Pending {}@{} DISCARDED", eldest.getValue(), eldest.getKey());
                        }
                        return shouldDiscard;
                    }
                };
        this.failedPeers =
                new LinkedHashMap<NodeID, Instant>(MAX_SIZE_FAILURE_RECORDS, 0.75f, true) {
                    @Override
        ...
        setSyncState(
                new DecidingSyncState(this.syncConfiguration, this, syncInformation, peerStatuses));
    }

    public void processStatus(MessageChannel sender, Status status) {
        logger.debug(
                "Receiving syncState from node {} block {} {}",
                sender.getPeerNodeID(),
                status.getBestBlockNumber(),
                HashUtil.shortHash(status.getBestBlockHash()));
    ...
    public void processSkeletonResponse(MessageChannel peer, SkeletonResponseMessage message) {
        ...
        if (isPending(messageId, messageType)) {
            ...
            peerScoringManager.recordEvent(
                    peer.getPeerNodeID(), null, EventType.UNEXPECTED_MESSAGE);
        }
    }
    ...
    public void processBlockHashResponse(MessageChannel peer, BlockHashResponseMessage message) {
        ...
        logger.debug(
                "Process block hash response from node {} hash {}",
                nodeID,
                HashUtil.shortHash(message.getHash()));
    ...
    public void processBlockHeadersResponse(
            MessageChannel peer, BlockHeadersResponseMessage message) {
    ...
    public void processNewBlockHash(MessageChannel peer, NewBlockHashMessage message) {
        NodeID nodeID = peer.getPeerNodeID();
        logger.debug(
                "Process new block hash from node {} hash {}",
                nodeID,
                HashUtil.shortHash(message.getBlockHash()));
        byte[] hash = message.getBlockHash();

        if (syncState instanceof DecidingSyncState
                && blockSyncService.getBlockFromStoreOrBlockchain(hash) == null) {
            peerStatuses.getOrRegisterPeer(nodeID);
    ...
    public void processBlockResponse(MessageChannel peer, BlockResponseMessage message) {
        NodeID nodeID = peer.getPeerNodeID();
        logger.debug(
                "Process block response from node {} block {} {}",
                nodeID,
                message.getBlock().getNumber(),
                message.getBlock().getShortHash());
        peerStatuses.getOrRegisterPeer(nodeID);
    ...
    public Long sendBodyRequest(@Nonnull BlockHeader header, NodeID peerId) {
        logger.debug(
                "Send body request block {} hash {} to peer {}",
                header.getNumber(),
                HashUtil.shortHash(header.getHash().getBytes()),
                peerId);

        BodyRequestMessage message =
                new BodyRequestMessage(++lastRequestId, header.getHash().getBytes());
    ...
    public void startSyncing(NodeID nodeID) {
        ...
        byte[] bestBlockHash =
                syncInformation.getPeerStatus(selectedPeerId).getStatus().getBestBlockHash();
        setSyncState(
                new CheckingBestHeaderSyncState(
                        this.syncConfiguration, this, syncInformation, bestBlockHash));
    }
    ...
    public void startDownloadingBodies(
        ...
        final long peerBestBlockNumber =
                selectedSkeleton.get(selectedSkeleton.size() - 1).getNumber();
        ...
        setSyncState(
                new DownloadingBodiesSyncState(
    ...
    @Override
    public void startDownloadingHeaders(
    ...
    public void startFindingConnectionPoint() {
        ...
        long bestBlockNumber =
                syncInformation.getPeerStatus(selectedPeerId).getStatus().getBestBlockNumber();
        setSyncState(
                new FindingConnectionPointSyncState(
                        this.syncConfiguration, this, syncInformation, bestBlockNumber));
    }
    ...
    int getNoAdvancedPeers() {
        ...
        return this.peerStatuses.countIf(
                s -> chainStatus.hasLowerTotalDifficultyThan(s.getStatus()));
    }
    ...
    public void setSelectedPeer(MessageChannel peer, Status status, long height) {
        ...
        FindingConnectionPointSyncState newState =
                new FindingConnectionPointSyncState(
                        this.syncConfiguration, this, syncInformation, height);
    ...
    private boolean isPending(long messageId, MessageType messageType) {
        return pendingMessages.containsKey(messageId)
                && pendingMessages.get(messageId) == messageType;
    }
    ...
    private class SyncInformationImpl implements SyncInformation {
        ...
        public boolean hasLowerDifficulty(NodeID nodeID) {
            ...
            // this works only for testing purposes, real status without difficulty don't reach this
            // far
            return (hasTotalDifficulty && nodeStatus.hasLowerTotalDifficultyThan(status))
                    || (!hasTotalDifficulty
                            && nodeStatus.getBestBlockNumber() < status.getBestBlockNumber());
        }

        @Override
        public BlockProcessResult processBlock(Block block, MessageChannel channel) {
            // this is a controled place where we ask for blocks, we never should look for missing
            // hashes
        ...
        @Override
        public boolean blockHeaderIsValid(
                @Nonnull BlockHeader header, @Nonnull BlockHeader parentHeader) {
