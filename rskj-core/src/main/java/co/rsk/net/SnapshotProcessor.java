...
public class SnapshotProcessor implements InternalService {
    ...
    public void processStateChunkResponse(SnapSyncState state, Peer peer, SnapStateChunkResponseMessage responseMessage) {
        ...
        try {
            final var snapStateChunkResponseMessageWriter = snapStateChunkResponseMessageStore.startWriting();

            snapStateChunkResponseMessageWriter.write(responseMessage, peer);
        ...
        try {
            final var snapStateChunkResponseMessageReader = snapStateChunkResponseMessageStore.startReading((nextMessage, peerNodeId) -> {
                ...
                if (!nextMessage.isComplete()) {
                    ...
                    final var foundPeer = peersInformation.getBestSnapPeerCandidates().stream().filter(p-> p.getPeerNodeID().toString().equals(peerNodeId)).findFirst().get();
                    executeNextChunkRequestTask(state, foundPeer);
                }
            });
