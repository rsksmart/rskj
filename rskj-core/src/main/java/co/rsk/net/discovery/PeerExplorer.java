...
public class PeerExplorer {
    ...
    private final Map<String, PeerDiscoveryRequest> pendingFindNodeRequests =
    ...
    public void handleMessage(DiscoveryEvent event) {
        ...
        if (type == DiscoveryMessageType.NEIGHBORS) {
            this.handleNeighborsMessage(
                    event.getAddress(), (NeighborsPeerMessage) event.getMessage());
        }
    }
    ...
    public void handlePong(InetSocketAddress pongAddress, PongPeerMessage message) {
        ...
        if (request != null && request.validateMessageResponse(pongAddress, message)) {
            ...
            if (challenge == null) {
                this.addConnection(
                        message,
                        request.getAddress().getHostString(),
    ...
    public void handleNeighborsMessage(
            InetSocketAddress neighborsResponseAddress, NeighborsPeerMessage message) {
        Node connectedNode = this.establishedConnections.get(message.getNodeId());

        if (connectedNode != null) {
            logger.debug("Neighbors received from [{}]", connectedNode.getHexIdShort());
            PeerDiscoveryRequest request =
                    this.pendingFindNodeRequests.remove(message.getMessageId());

            if (request != null
                    && request.validateMessageResponse(neighborsResponseAddress, message)) {
                List<Node> nodes =
    ...
    public NeighborsPeerMessage sendNeighbors(
            InetSocketAddress nodeAddress, List<Node> nodes, String id) {
        List<Node> nodesToSend = getRandomizeLimitedList(nodes, MAX_NODES_PER_MSG, 5);
        NeighborsPeerMessage sendNodesMessage =
                NeighborsPeerMessage.create(nodesToSend, id, this.key, networkId);
    ...
    public void purgeRequests() {
        List<PeerDiscoveryRequest> oldPingRequests =
                removeExpiredRequests(this.pendingPingRequests);
    ...
    private void checkPeersPulse(List<Node> closestNodes) {
        List<Node> nodesToCheck =
                this.getRandomizeLimitedList(closestNodes, MAX_NODES_TO_CHECK, 10);
    ...
    private List<PeerDiscoveryRequest> removeExpiredRequests(
            Map<String, PeerDiscoveryRequest> pendingRequests) {
        List<PeerDiscoveryRequest> requests =
                pendingRequests.values().stream()
                        .filter(PeerDiscoveryRequest::hasExpired)
                        .collect(Collectors.toList());
    ...
    private void removeExpiredChallenges(List<PeerDiscoveryRequest> peerDiscoveryRequests) {
        peerDiscoveryRequests.stream()
                .forEach(r -> challengeManager.removeChallenge(r.getMessageId()));
    }
        ...
        if (!StringUtils.equals(senderNode.getHexId(), this.localNode.getHexId())) {
            ...
            if (result.isSuccess()) {
                ...
                this.challengeManager.startChallenge(
                        result.getAffectedEntry().getNode(), senderNode, this);
            }
        }
    }
    ...
    private List<Node> getRandomizeLimitedList(
            List<Node> nodes, int maxNumber, int randomElements) {
