...
public class TxValidatorMaximumGasPriceValidator implements TxValidatorStep {
    ...
    @VisibleForTesting
    static final BigInteger GAS_PRICE_CAP_MULTIPLIER = BigInteger.valueOf(80);
    ...
    public TransactionValidationResult validate(Transaction tx, @Nullable AccountState state, BigInteger gasLimit, Coin minimumGasPrice, long bestBlockNumber, boolean isFreeTx) {
        ...
        if (minimumGasPrice.equals(Coin.ZERO)) {
            return TransactionValidationResult.ok();
        }

        Coin gasPriceCap = minimumGasPrice.multiply(GAS_PRICE_CAP_MULTIPLIER);
        if (tx.getGasPrice().compareTo(gasPriceCap) <= 0) {
            return TransactionValidationResult.ok();
        }

        return TransactionValidationResult.withError("transaction's gas price exceeds cap of " + gasPriceCap);
    }
