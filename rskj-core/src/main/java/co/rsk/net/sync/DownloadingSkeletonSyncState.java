...
public class DownloadingSkeletonSyncState extends BaseSyncState {
    ...
    public DownloadingSkeletonSyncState(
    ...
    public void newSkeleton(List<BlockIdentifier> skeleton, MessageChannel peer) {
        ...
        if (skeleton.size() < 2) {
            syncInformation.reportEvent(
                    "Invalid skeleton received from node {}", EventType.INVALID_MESSAGE, peerId, peerId);
        ...
        if (timeElapsed.compareTo(syncConfiguration.getTimeoutWaitingRequest()) >= 0) {
            candidates.stream()
                    ...
                    .forEach(
                            p ->
                                    syncInformation.reportEvent(
                                            "Timeout waiting skeleton from node {}", EventType.TIMEOUT_MESSAGE, p, p));
