...
/**
 * This is mostly a workaround because SyncProcessor needs to access MessageChannel instances. TODO(mc) remove this
 * after the logical node abstraction is created, since it will wrap things such as the underlying communication
 * channel.
 */
public class PeersInformation {
    ...
    public PeersInformation(
            SyncInformation syncInformation, ChannelManager channelManager, SyncConfiguration syncConfiguration) {
        this.channelManager = channelManager;
        this.syncInformation = syncInformation;
        this.syncConfiguration = syncConfiguration;
        this.peerComparator =
                ((Comparator<Map.Entry<NodeID, SyncPeerStatus>>) this::comparePeerFailInstant)
    ...
    public int countIf(Predicate<SyncPeerStatus> predicate) {
        long count = peerStatuses.values().stream().filter(predicate).count();
    ...
    public Optional<NodeID> getBestPeer() {
        return getCandidatesStream().max(this.peerComparator).map(Map.Entry::getKey);
    }

    private Stream<Map.Entry<NodeID, SyncPeerStatus>> getCandidatesStream() {
        Set<NodeID> activeNodes =
                channelManager.getActivePeers().stream().map(Channel::getNodeId).collect(Collectors.toSet());
    ...
    private int comparePeerFailInstant(
            Map.Entry<NodeID, SyncPeerStatus> entry, Map.Entry<NodeID, SyncPeerStatus> other) {
    ...
    private int comparePeerScoring(Map.Entry<NodeID, SyncPeerStatus> entry, Map.Entry<NodeID, SyncPeerStatus> other) {
