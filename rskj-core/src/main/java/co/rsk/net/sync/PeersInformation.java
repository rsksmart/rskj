 ...
 * TODO(mc) remove this after the logical node abstraction is created, since it will wrap things
 * such as the underlying communication channel.
 */
public class PeersInformation {
    ...
    public PeersInformation(
        ...
        this.peerComparator =
                ((Comparator<Map.Entry<NodeID, SyncPeerStatus>>) this::comparePeerFailInstant)
                        // TODO reenable when unprocessable blocks stop being marked as invalid
                        // blocks
    ...
    private Stream<Map.Entry<NodeID, SyncPeerStatus>> getCandidatesStream() {
        Set<NodeID> activeNodes =
                channelManager.getActivePeers().stream()
                        .map(Channel::getNodeId)
