...
public class DownloadingBodiesSyncState extends BaseSyncState {
    ...
    public void tick(Duration duration) {
        ...
        updatedNodes.forEach(
                k -> timeElapsedByPeer.put(k, timeElapsedByPeer.get(k).plus(duration)));
    ...
    private void handleTimeoutMessage(NodeID peerId) {
        syncInformation.reportEvent(
                "Timeout waiting body from node {}", EventType.TIMEOUT_MESSAGE, peerId, peerId);
        Long messageId = messagesByPeers.remove(peerId);
    ...
    /**
     * This method finds with the skeletons from each node, the segments we can divide chunks. Each
     * chunk belongs to a single segment, and each node associated to a segment can answer for each
     * block inside the chunks belonging to a segment. Also each node on a superior segment can
     * answer for every block on lower segments. The idea is to find the "min common chunks" between
     * nodes to find when a new segment starts
     */
        ...
        for (; chunkNumber < pendingHeaders.size(); chunkNumber++) {
            ...
            if (prevNodes.size() != nodes.size()) {
                ...
                List<NodeID> insertedNodes =
                        prevNodes.stream()
                                .filter(k -> !filteringNodes.contains(k))
                                .collect(Collectors.toList());
    ...
    private void insertSegment(
            Deque<Integer> segmentChunks, List<NodeID> nodes, Integer segmentNumber) {
    ...
    private void tryRequestBody(NodeID peerId, BlockHeader header) {
        ...
        if (messageId != null) {
            ...
            // since a message could fail to be delivered we have to discard peer if can't be
            // reached
            clearPeerInfo(peerId);
            syncEventsHandler.onSyncIssue(
                    "Channel failed to sent on {} to {}", this.getClass(), peerId);
        }
    }
