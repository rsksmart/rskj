...
import co.rsk.net.messages.BodyRequestMessage;
...
public class NodeMessageHandler implements MessageHandler, Runnable {
    ...
    public synchronized void processMessage(final MessageChannel sender, @Nonnull final Message message) {
        ...
        loggerMessageProcess.debug(
                "Message[{}] processed after [{}] nano.", message.getMessageType(), System.nanoTime() - start);
    }

    @Override
    public void postMessage(MessageChannel sender, Message message) throws InterruptedException {
        logger.trace(
                "Start post message (queue size {}) (message type {})", this.queue.size(), message.getMessageType());
    ...
    /**
     * isValidBlock validates if the given block meets the minimum criteria to be processed: The PoW should be valid and
    ...
    private void processBlockHeadersRequestMessage(
            @Nonnull final MessageChannel sender, @Nonnull final BlockHeadersRequestMessage message) {
        final long requestId = message.getId();
    ...
    private void processBlockHeadersResponseMessage(
            @Nonnull final MessageChannel sender, @Nonnull final BlockHeadersResponseMessage message) {
