 ...
 *
 * <p>The properties are taken from different sources and merged in the following order (the config
 * option from the next source overrides option from previous): - resource rskj.conf : normally used
 * as a reference config with default values and shouldn't be changed - system property : each
 * config entry might be altered via -D VM option - [user dir]/config/rskj.conf - config specified
 * with the -Drsk.conf.file=[file.conf] VM option - CLI options
 *
...
public abstract class SystemProperties {
    ...
    private static final String PROPERTY_RPC_WEBSOCKET_ADDRESS =
            "rpc.providers.web.ws.bind_address";
    ...
    public ActivationConfig getActivationConfig() {
        if (activationConfig == null) {
            activationConfig =
                    ActivationConfig.read(configFromFiles.getConfig(PROPERTY_BLOCKCHAIN_CONFIG));
        }
                                    ...
                                    getGenesisFederationPublicKeys()
                                            .orElse(
                                                    BridgeRegTestConstants
                                                            .REGTEST_FEDERATION_PUBLIC_KEYS));
                    break;
                default:
                    throw new RuntimeException(
                            String.format("Unknown network name '%s'", netName()));
            }
        }
    ...
    public boolean isPeerDiscoveryEnabled() {
        return discoveryEnabled == null
                ? configFromFiles.getBoolean("peer.discovery.enabled")
                : discoveryEnabled;
    }
    ...
    private Node parsePeer(ConfigObject configObject) {
        ...
        if (configObject.get("ip") != null) {
            ...
            if (configObject.toConfig().hasPath("nodeId")) {
                ...
                throw new RuntimeException(
                        "Invalid config nodeId '" + nodeId + "' at " + configObject);
            }
        ...
        list.stream()
                ...
                .forEach(
                        config -> {
                            ...
                            byte[] nodeId =
                                    nodeIdData != null ? Hex.decode(nodeIdData.trim()) : null;
    ...
    public String rootHashStart() {
        return configFromFiles.hasPath("root.hash.start")
                ? configFromFiles.getString("root.hash.start")
                : null;
    }
    ...
    private String getMyPublicIpFromRemoteService() {
        try {
            logger.info(
                    "Public IP wasn't set or resolved, using checkip.amazonaws.com to identify it...");
            ...
            if (publicIp == null || publicIp.trim().isEmpty()) {
                logger.warn(
                        "Unable to retrieve public IP from checkip.amazonaws.com {}.", publicIp);
        ...
        if (getBindAddress().isAnyLocalAddress()) {
            throw new RuntimeException(
                    "Wildcard on bind address it's not allowed as fallback for public IP "
                            + bindAddress);
        }
    ...
    public long scoringAddressesPunishmentMaximumDuration() {
        ...
        return TimeUnit.MINUTES.toMillis(
                getLong("scoring.addresses.maximum", TimeUnit.DAYS.toMinutes(7)));
    }
