...
import static org.ethereum.vm.OpCode.Tier.BASE_TIER;
import static org.ethereum.vm.OpCode.Tier.EXT_TIER;
import static org.ethereum.vm.OpCode.Tier.HIGH_TIER;
import static org.ethereum.vm.OpCode.Tier.LOW_TIER;
import static org.ethereum.vm.OpCode.Tier.MID_TIER;
import static org.ethereum.vm.OpCode.Tier.SPECIAL_TIER;
import static org.ethereum.vm.OpCode.Tier.VERY_LOW_TIER;
import static org.ethereum.vm.OpCode.Tier.ZERO_TIER;

import java.util.HashMap;
import java.util.Map;

/**
 * Instruction set for the Ethereum Virtual Machine See Yellow Paper: http://www.gavwood.com/Paper.pdf - Appendix G.
...
public enum OpCode {
    ...
    /**
     * (0x3d and 0x3e) A mechanism to allow returning arbitrary-length data. After a call, return data is kept inside a
     * virtual buffer from which the caller can copy it (or parts thereof) into memory. At the next call, the buffer is
     * overwritten.
     */
    ...
    CALL(
            ...
            7,
            1,
            SPECIAL_TIER), //       [out_data_size] [out_data_start] [in_data_size] [in_data_start] [value] [to_addr]
    // [gas] CALL
    /** (0xf2) Calls self, but grabbing the code from the TO argument instead of from one's own address */
    CALLCODE(0xf2, 7, 1, SPECIAL_TIER),
    /** (0xf3) Halt execution returning output data */
    RETURN(0xf3, 2, 0, ZERO_TIER),

    /**
     * (0xf4) similar in idea to CALLCODE, except that it propagates the sender and value from the parent scope to the
     * child scope, ie. the call created has the same sender and value as the original call. also the Value parameter is
     * omitted for this code
     */
    ...
    /**
     * (0xfc) Halt execution as an invalid opcode setting version==256 assures it's never considered valid. (because
     * scriptVersion range is 0..255)
     */
    HEADER(0xfc, 0, 0, ZERO_TIER, 256),

    /**
     * opcode that can be used to call another contract (or itself) while disallowing any modifications to the state
     * during the call (and its subcalls, if present). Any opcode that attempts to perform such a modification (see
     * below for details) will result in an exception instead of performing the modification.
     */
    STATICCALL(0xfa, 6, 1, SPECIAL_TIER),
    /**
     * (0xfd) The `REVERT` instruction will stop execution, roll back all state changes done so far and provide a
     * pointer to a memory section, which can be interpreted as an error code or message. While doing so, it will not
