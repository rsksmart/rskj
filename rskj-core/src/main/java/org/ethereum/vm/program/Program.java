...
public class Program {
    ...
    /**
     * This attribute defines the number of recursive calls allowed in the EVM Note: For the JVM to reach this level
     * without a StackOverflow exception, ethereumj may need to be started with a JVM argument to increase the stack
     * size. For example: -Xss10m
     */
    ...
    public Program(
        ...
        if (isLogEnabled) {
            logger.warn(
                    "WARNING! VM logging is enabled. This will make the VM 200 times slower. Do not use in"
                        + " production.");
        }

        if (isGasLogEnabled) {
            gasLogger.warn(
                    "WARNING! Gas logging is enabled. This will the make VM 200 times slower. Do not use in"
                        + " production.");
        }
    ...
    private ProgramResult getProgramResult(
        ...
        if (programResult.getException() != null || programResult.isRevert()) {
            ...
            if (afterSpend < 0) {
                ...
                programResult.setException(
                        ExceptionHelper.tooLargeContractSize(Constants.getMaxContractSize(), codeLength));
            } else {
     ...
     *
     * <p>- Normal calls invoke a specified contract which updates itself - Stateless calls invoke code from another
     * contract, within the context of the caller
     *
    ...
    public void callToAddress(MessageCall msg) {
        ...
        InternalTransaction internalTx =
                addInternalTx(null, getGasLimit(), senderAddress, contextAddress, endowment, programCode, "call");
        ...
        if (!isEmpty(programCode)) {
            callResult =
                    executeCode(
                            msg, contextAddress, contextBalance, internalTx, track, programCode, senderAddress, data);
        } else {
            track.commit();
            callResult = true;
        ...
        if (logger.isTraceEnabled() || listener != null) {
            ...
            if (memory.size() > 320) {
                ...
                for (int i = 0; i < memory.size(); ++i) {
                    ...
                    if ((i + 1) % 16 == 0) {
                        String tmp =
                                format("[%4s]-[%4s]", Integer.toString(i - 15, 16), Integer.toString(i, 16))
                                        .replace(" ", "0");
    ...
    public void callToPrecompiledAddress(MessageCall msg, PrecompiledContract contract) {
        ...
        if (contract instanceof Bridge || contract instanceof RemascContract || contract instanceof NativeContract) {
            ...
            InternalTransaction internalTx =
                    addInternalTx(
                            null, getGasLimit(), senderAddress, contextAddress, endowment, EMPTY_BYTE_ARRAY, "call");
    ...
    public static class ExceptionHelper {
        ...
        public static OutOfGasException notEnoughOpGas(OpCode op, long opGas, long programGas) {
            return new OutOfGasException(
                    "Not enough gas for '%s' operation executing: opGas[%d], programGas[%d];", op, opGas, programGas);
        }
        ...
        public static OutOfGasException gasOverflow(BigInteger actualGas, BigInteger gasLimit) {
            return new OutOfGasException(
                    "Gas value overflow: actualGas[%d], gasLimit[%d];", actualGas.longValue(), gasLimit.longValue());
        }

        public static OutOfGasException gasOverflow(long actualGas, BigInteger gasLimit) {
            return new OutOfGasException(
                    "Gas value overflow: actualGas[%d], gasLimit[%d];", actualGas, gasLimit.longValue());
        }
        ...
        public static AddressCollisionException addressCollisionException(RskAddress address) {
            return new AddressCollisionException(
                    "Trying to create a contract with existing contract address: 0x" + address.toString());
        }
    }
