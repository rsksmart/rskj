...
/**
 * Service in charge of the implementations of all the Signature related
 * functionality.
 * Functions:
 * - Sign data (future)
...
public interface Secp256k1Service {
     ...
     * ECKey
     * containing the public key that was used to sign it. This can then be compared
     * to the expected public key to
     ...
     * keys is the correct one. Because
     * the key recovery operation yields multiple potential keys, the correct key
     * must either be stored alongside the
     * signature, or you must be willing to try each recId in turn until you find
     * one that outputs the key you are
     * expecting.
     * </p>
     *
     * <p>
     ...
     * loop from 0 to 3, and if the
     * output is null OR a key that is not the one you expect, you try again with
     * the next recId.
     * </p>
     *
    ...
    byte[] add(byte[] data) throws VMException;

    byte[] mul(byte[] data) throws VMException;
}