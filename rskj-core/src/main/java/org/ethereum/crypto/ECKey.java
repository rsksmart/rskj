 ...
 *
 * <p>A key can be <i>compressed</i> or <i>uncompressed</i>. This refers to whether the public key
 * is represented when encoded into bytes as an (x, y) coordinate on the elliptic curve, or whether
 * it's represented as just an X co-ordinate and an extra byte that carries a sign bit. With the
 * latter form the Y coordinate can be calculated dynamically, however, <b>because the binary
 * serialization is different the address of a key changes if its compression status is changed</b>.
 * If you deviate from the defaults it's important to understand this: money sent to a compressed
 * version of the key will have a different address to the same key in uncompressed form. Whether a
 * public key is compressed or not is recorded in the SEC binary serialisation format, and preserved
 * in a flag in this class so round-tripping preserves state. Unless you're working with old
 * software or doing unusual things, you can usually ignore the compressed/uncompressed distinction.
 * This code is borrowed from the bitcoinj project and altered to fit Ethereum.<br>
 * See <a
 * href="https://github.com/bitcoinj/bitcoinj/blob/master/core/src/main/java/com/google/bitcoin/core/ECKey.java">
...
public class ECKey {
    ...
    /**
     * Equal to CURVE.getN().shiftRight(1), used for canonicalising the S value of a signature. If
     * you aren't sure what this is about, you can ignore it.
     */
    ...
    /**
     * Generates an entirely new keypair. Point compression is used so the resulting public key will
     * be 33 bytes (32 for the co-ordinate and 1 byte to represent the y bit).
     */
    public ECKey() {
        this(secureRandom);
    }
    ...
    /**
     * Generates an entirely new keypair with the given {@link SecureRandom} object. Point
     * compression is used so the resulting public key will be 33 bytes (32 for the co-ordinate and
     * 1 byte to represent the y bit).
     *
     * @param secureRandom -
     */
    ...
    /**
     * Creates an ECKey given the private key only. The public key is calculated from it (this is
     * slow). Note that the resulting public key is compressed.
     *
     * @param privKey -
     * @return -
     */
    ...
    /**
     * Creates an ECKey given the private key only. The public key is calculated from it (this is
     * slow). The resulting public key is compressed.
     *
     * @param privKeyBytes -
     * @return -
     */
     ...
     * <b>not</b> the same as the pubKeyHash/address.
     *
     * @return -
     */
    ...
    public static class ECDSASignature {
         ...
         * @return -
         */
        public static ECDSASignature fromComponentsWithRecoveryCalculation(
                byte[] r, byte[] s, byte[] hash, byte[] pub) {
         ...
         * order, if necessary. This is required because for every signature (r,s) the signature (r,
         * -s (mod N)) is a valid signature of the same message. However, we dislike the ability to
         * modify the bits of a Ethereum transaction after it's been signed, as that violates
         * various assumed invariants. Thus in future only one of those forms will be considered
         * legal and the other will be banned.
         *
         * @return -
         */
        public ECDSASignature toCanonicalised() {
            if (s.compareTo(HALF_CURVE_ORDER) > 0) {
                // The order of the curve is the number of valid points that exist on that curve. If
                // S is in the upper
     ...
     */
    public static ECKey signatureToKey(byte[] messageHash, ECDSASignature signature)
     ...
     */
    @Nullable
    public static ECKey recoverFromSignature(
            int recId, ECDSASignature sig, byte[] messageHash, boolean compressed) {
