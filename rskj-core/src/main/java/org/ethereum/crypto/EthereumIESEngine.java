...
/**
 * Support class for constructing integrated encryption cipher for doing basic message exchanges on top of key agreement
 * ciphers. Follows the description given in IEEE Std 1363a with a couple of changes specific to Ethereum: - Hash the
 * MAC key before use - Include the encryption iv in the MAC computation
 */
public class EthereumIESEngine {
    ...
    /**
     * set up for use with stream mode, where the key derivation function is used to provide a stream of bytes to xor
     * with the message.
     *
    ...
    public EthereumIESEngine(
            BasicAgreement agree, DerivationFunction kdf, Mac mac, Digest hash, BufferedBlockCipher cipher) {
        this.agree = agree;
        this.kdf = kdf;
        this.mac = mac;
        this.hash = hash;
     ...
     */
    public void init(
            boolean forEncryption, CipherParameters privParam, CipherParameters pubParam, CipherParameters params) {
    ...
    private byte[] decryptBlock(byte[] inEnc, int inOff, int inLen, byte[] macData) throws InvalidCipherTextException {
        byte[] m = null;
        byte[] k = null;
        byte[] k1 = null;
        byte[] k2 = null;
