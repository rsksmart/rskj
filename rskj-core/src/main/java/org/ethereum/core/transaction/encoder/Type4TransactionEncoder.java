...
public class Type4TransactionEncoder implements TransactionEncoder {
    ...
    private byte[][] encodeUnsignedFields(Transaction transaction) {
        if (transaction.getMaxPriorityFeePerGas() == null || transaction.getMaxFeePerGas() == null) {
            throw new IllegalStateException("Type 4 transaction requires maxPriorityFeePerGas and maxFeePerGas");
        }
        if (transaction.getAuthorizationList() == null || transaction.getAuthorizationList().isEmpty()) {
            throw new IllegalStateException("Type 4 transaction requires a non-empty authorization list");
        }
