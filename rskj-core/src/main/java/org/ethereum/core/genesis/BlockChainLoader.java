...
public class BlockChainLoader {
    ...
    // this is here since every time blockchain is loaded needs the right genesis version
    private void tryUpdateGenesisUnitrie() {
        if (!config.getBlockchainConfig().getConfigForBlock(0).isRskipUnitrie()) {
            byte[] stateRootHash = trieConverter.getOrchidAccountTrieRoot((TrieImpl) repository.getMutableTrie().getTrie());
            stateRootTranslator.put(new Keccak256(stateRootHash), new Keccak256(genesis.getStateRoot()));
