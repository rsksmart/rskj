...
public class BlockHeaderV2 extends BlockHeaderV1 {
    ...
    public BlockHeaderV2(byte[] parentHash, byte[] unclesHash, RskAddress coinbase, byte[] stateRoot, byte[] txTrieRoot,
                         ...
                         boolean includeForkDetectionData, byte[] ummRoot, byte[] superChainDataHash,
                         SuperBlockResolver isSuper, short[] txExecutionSublistsEdges, boolean compressed) {
        super(parentHash, unclesHash, coinbase, stateRoot, txTrieRoot, receiptTrieRoot, extensionData, difficulty,
                number, gasLimit, gasUsed, timestamp, extraData, paidFees, bitcoinMergedMiningHeader,
                bitcoinMergedMiningMerkleProof, bitcoinMergedMiningCoinbaseTransaction, mergedMiningForkDetectionData,
                minimumGasPrice, uncleCount, sealed, useRskip92Encoding, includeForkDetectionData, ummRoot,
                superChainDataHash, isSuper, txExecutionSublistsEdges, compressed);
    }

    @Override
    public byte getVersion() { return 0x2; }

    public byte[] getSuperChainDataHash() {
        return this.getExtension().getSuperChainDataHash();
    }
    ...
    public void setSuperChainDataHash(byte[] superChainDataHash) {
        ...
        this.getExtension().setSuperChainDataHash(superChainDataHash);
    ...
    protected BlockHeaderExtensionV1 createExtension(boolean compressed,
        ...
        return compressed
                ...
                : new BlockHeaderExtensionV2(extensionData, txExecutionSublistsEdges, superChainDataHash);
    }
    ...
    public void addExtraFieldsToEncodedHeader(boolean usingCompressedEncoding, List<byte[]> fieldsToEncode) {
        ...
        if (!usingCompressedEncoding) {
            addSuperChainDataHash(fieldsToEncode);
        }
    }
