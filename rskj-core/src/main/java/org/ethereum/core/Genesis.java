...
/**
 * The genesis block is the first block in the chain and has fixed values according to the protocol specification. The
 * genesis block is 13 items, and is specified thus:
 *
 * <p>( zerohash_256 , SHA3 RLP () , zerohash_160 , stateRoot, 0, 2^22 , 0, 0, 1000000, 0, 0, 0, SHA3 (42) , (), () )
 *
 * <p>- Where zerohash_256 refers to the parent hash, a 256-bit hash which is all zeroes; - zerohash_160 refers to the
 * coinbase address, a 160-bit hash which is all zeroes; - 2^22 refers to the difficulty; - 0 refers to the timestamp
 * (the Unix epoch); - the transaction trie root and extradata are both 0, being equivalent to the empty byte array. -
 * The sequences of both uncles and transactions are empty and represented by (). - SHA3 (42) refers to the SHA3 hash of
 * a byte array of length one whose first and only byte is of value 42. - SHA3 RLP () value refers to the hash of the
 * uncle lists in RLP, both empty lists.
 *
...
public class Genesis extends Block {
    ...
    /**
     * WORKAROUND. This is overrode because the Genesis' parent hash is an empty byte array, which isn't a valid
     * Keccak256 hash. For encoding purposes, the empty byte array is used instead.
     */
    @Override
