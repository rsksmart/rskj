...
public interface Repository extends AccountInformationProvider {
     ...
     *     <p>This method creates an account, but is DOES NOT create a contract. To create a contract, internally the
     *     account node is extended with a root node for storage. To avoid creating the root node for storage each time
     *     a storage cell is added, we pre-create the storage node when we know the account will become a contract. This
     *     is done in setupContract(). Note that we can't use the length or existence of the code node for this, because
     *     a contract's code can be empty!
     */
     ...
     *
     * <p>This method returns null if there is no code at the address. It may return the empty array for contracts that
     * have installed zero code on construction. (not checked)
     */
    @Override
    @Nullable
