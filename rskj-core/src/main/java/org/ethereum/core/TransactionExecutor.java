...
public class TransactionExecutor {
    ...
    /**
     * Do all the basic validation, if the executor will be ready to run the transaction at the end
    ...
    public boolean init() {
        ...
        boolean cumulativeGasReached =
                txGasLimit.add(BigInteger.valueOf(gasUsedInTheBlock)).compareTo(curBlockGasLimit)
                        > 0;
        ...
        if (txGasLimit.compareTo(BigInteger.valueOf(basicTxCost)) < 0) {
            execError(
                    String.format(
                            "Not enough gas for transaction execution: Require: %s Got: %s",
                            basicTxCost, txGasLimit));
            return false;
        }
        ...
        if (isNotEqual(reqNonce, txNonce)) {
            ...
            if (logger.isWarnEnabled()) {
                ...
                logger.warn(
                        "Tx Included in the following block: {}",
                        this.executionBlock.getShortDescr());
            }

            execError(
                    String.format("Invalid nonce: required: %s , tx.nonce: %s", reqNonce, txNonce));
            return false;
        }
        ...
        if (!isCovers(senderBalance, totalCost)) {
            ...
            logger.warn(
                    "Not enough cash: Require: {}, Sender cash: {}, tx {}",
                    totalCost,
                    senderBalance,
                    tx.getHash());
            logger.warn("Transaction Data: {}", tx);
            ...
            execError(
                    String.format(
                            "Not enough cash: Require: %s, Sender cash: %s",
                            totalCost, senderBalance));
        ...
        if (receiveAddress != null
                && !Arrays.equals(receiveAddress, EMPTY_BYTE_ARRAY)
                && receiveAddress.length > Constants.getMaxAddressByteLength()) {
            logger.warn(
                    "Receiver address to long: size: {}, tx {}",
        ...
        if (!tx.acceptTransactionSignature(constants.getChainId())) {
            logger.warn(
                    "Transaction {} signature not accepted: {}", tx.getHash(), tx.getSignature());
            logger.warn("Transaction Data: {}", tx);
            logger.warn("Tx Included in the following block: {}", this.executionBlock);
            ...
            panicProcessor.panic(
                    "invalidsignature",
                    String.format(
                            "Transaction %s signature not accepted: %s",
    ...
    private void call() {
        ...
        precompiledContract =
                precompiledContracts.getContractForAddress(
                        activations, DataWord.valueOf(targetAddress.getBytes()));

        if (precompiledContract != null) {
            Metric metric = profiler.start(Profiler.PROFILING_TYPE.PRECOMPILED_CONTRACT_INIT);
            precompiledContract.init(
                    tx, executionBlock, track, blockStore, receiptStore, result.getLogInfoList());
            ...
            if (!localCall && txGasLimit.compareTo(BigInteger.valueOf(requiredGas)) < 0) {
                ...
                execError(
                        String.format(
                                "Out of Gas calling precompiled contract 0x%s, required: %d, left: %s ",
            ...
            if (isEmpty(code)) {
                ...
                ProgramInvoke programInvoke =
                        programInvokeFactory.createProgramInvoke(
                                tx, txindex, executionBlock, cacheTrack, blockStore);
    ...
    public void go() {
        ...
        // Set the deleted accounts in the block in the remote case there is a CREATE2 creating a
        // deleted account
                ...
                if (mEndGas.compareTo(BigInteger.valueOf(returnDataGasValue)) < 0) {
                    ...
                    program.setRuntimeFailure(
                            Program.ExceptionHelper.tooLargeContractSize(
                                    Constants.getMaxContractSize(), createdContractSize));
                    result = program.getResult();
                    result.setHReturn(EMPTY_BYTE_ARRAY);
                } else {
        ...
        if (receipt == null) {
            ...
            receipt.setStatus(
                    executionError.isEmpty()
                            ...
                            : TransactionReceipt.FAILED_STATUS);
        }
        return receipt;
    }
    ...
    public void finalization() {
        ...
        List<LogInfo> notRejectedLogInfos =
                result.getLogInfoList().stream()
                        .filter(logInfo -> !logInfo.isRejected())
                        .collect(Collectors.toList());

        TransactionExecutionSummary.Builder summaryBuilder =
                TransactionExecutionSummary.builderFor(tx)
                        .gasLeftover(mEndGas)
        ...
        logger.trace(
                "Pay total refund to sender: [{}], refund val: [{}]",
                tx.getSender(),
                summary.getRefund());
    ...
    /**
     * This extracts the trace to an object in memory. Refer to {@link
     * org.ethereum.vm.VMUtils#saveProgramTraceFile} for a way to saving the trace to a file.
     */
    public void extractTrace(ProgramTraceProcessor programTraceProcessor) {
        if (program != null) {
            ProgramTrace trace =
                    program.getTrace().result(result.getHReturn()).error(result.getException());
