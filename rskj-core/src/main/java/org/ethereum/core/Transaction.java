 ...
 * external to Ethereum. An external actor can be a person (via a mobile device or desktop computer)
 * or could be from a piece of automated software running on a server. There are two types of
 * transactions: those which result in message calls and those which result in the creation of new
 * contracts.
 */
public class Transaction {
    ...
    public Transaction(
            long nonce, long gasPrice, long gas, String to, long value, byte[] data, byte chainId) {
        this(
    ...
    public Transaction(
        ...
        this(
                ...
                gasPrice,
                gasLimit,
                data == null ? null : Hex.decode(data),
                chainId);
    }
    ...
    // "return (this.isContractCreation() ? GasCost.TRANSACTION_CREATE_CONTRACT :
    // GasCost.TRANSACTION)
    ...
    public long transactionCost(Constants constants, ActivationConfig.ForBlock activations) {
        ...
        return (this.isContractCreation()
                        ? GasCost.TRANSACTION_CREATE_CONTRACT
                        : GasCost.TRANSACTION)
    ...
    public Coin getGasPrice() {
        // some blocks have zero encoded as null, but if we altered the internal field then
        // re-encoding the value would
    ...
    public boolean acceptTransactionSignature(byte currentChainId) {
        ...
        if (signature == null
                || !signature.validateComponents()
                || signature.s.compareTo(SECP256K1N_HALF) >= 0) {
            return false;
        }
                ...
                v =
                        RLP.encodeByte(
                                (byte)
                                        (chainId == 0
                                                ? signature.v
                                                : (signature.v - LOWER_REAL_V)
                                                        + (chainId * 2 + CHAIN_ID_INC)));
    ...
    public BigInteger getNonceAsInteger() {
        return (this.getNonce() == null)
                ? null
                : BigIntegers.fromUnsignedByteArray(this.getNonce());
    }
