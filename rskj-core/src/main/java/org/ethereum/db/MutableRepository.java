...
public class MutableRepository implements Repository {
            ...
            return new Iterator<DataWord>() {
                ...
                public boolean hasNext() {
                    ...
                    while (storageIterator.hasNext()) {
                        Trie.IterationElement storageIterationElement = storageIterator.next();
                        byte[] storageNodeExpandedKey = storageIterationElement.getExpandedPath();
                        if (storageNodeExpandedKey.length == (STORAGE_PREFIX.length + MutableRepository.SECURE_KEY_SIZE + DataWord.LENGHT_IN_BYTES) * Byte.SIZE - 1) { // -1 b/c the first bit is implicit in the storage node
                            byte[] storageExpandedKeySuffix = Arrays.copyOfRange(storageNodeExpandedKey, storageNodeExpandedKey.length - DataWord.LENGHT_IN_BYTES * Byte.SIZE, storageNodeExpandedKey.length);
                            currentStorageKey = new DataWord(PathEncoder.encode(storageExpandedKeySuffix));
                            return true;
                        }
                    }
                    return false;
    ...
    public synchronized Set<RskAddress> getAccountsKeys() {
        ...
        while (preOrderIterator.hasNext()) {
            Trie.IterationElement iterationElement = preOrderIterator.next();
            byte[] expandedPath = iterationElement.getExpandedPath();
            if (expandedPath.length == (1 + MutableRepository.SECURE_KEY_SIZE + RskAddress.LENGTH_IN_BYTES) * Byte.SIZE) {
                byte[] expandedAddress = Arrays.copyOfRange(expandedPath, expandedPath.length - RskAddress.LENGTH_IN_BYTES * Byte.SIZE, expandedPath.length);
                byte[] address = PathEncoder.encode(expandedAddress);
