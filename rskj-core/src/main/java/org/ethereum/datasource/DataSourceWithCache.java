...
public class DataSourceWithCache implements KeyValueDataSource {
    ...
    private void delete(ByteArrayWrapper wrappedKey) {
        ...
        // a null value means we know for a fact that the key doesn't exist in the underlying store,
        // so this is a noop
    ...
    public Set<byte[]> keys() {
        ...
        Stream<ByteArrayWrapper> committedKeys =
                committedCache.entrySet().stream()
        ...
        Set<ByteArrayWrapper> knownKeys =
                Stream.concat(Stream.concat(baseKeys, committedKeys), uncommittedKeys)
                        .collect(Collectors.toSet());
    ...
    public void updateBatch(
            Map<ByteArrayWrapper, byte[]> rows, Set<ByteArrayWrapper> keysToRemove) {
