 ...
 *
 * <p>The purpose of RLP is to encode arbitrarily nested arrays of binary data, and RLP is the main
 ...
 *
 * <p>For example, an empty string is an item, as is the string containing the word "cat", a list
...
public class RLP {
     ...
     * place to put the cutoff - also, that's where Bitcoin's varint does the cutof
     */
    ...
    public static BigInteger decodeBigInteger(byte[] data, int index) {
        RLPElementView info =
                RLPElementView.calculateFirstElementInfo(
                        ByteBuffer.wrap(data, index, data.length - index));
    ...
    public static RLPList decodeList(byte[] msgData) {
        ...
        if (decoded.size() != 1) {
            throw new IllegalArgumentException(
                    String.format("Expected one RLP item but got %d", decoded.size()));
        }
    ...
    public static RLPElement decode2OneItem(@CheckForNull byte[] msgData, int startPos) {
        ...
        return RLPElementView.calculateFirstElementInfo(
                        ByteBuffer.wrap(msgData, startPos, msgData.length - startPos))
                .getOrCreateElement();
    }
    ...
    private static void fullTraverse(
            @Nonnull ByteBuffer msgData, @Nonnull ArrayList<RLPElement> rlpList) {
        ...
        try {
            ...
            throw new RuntimeException(
                    "RLP wrong encoding (" + ByteBufferUtil.toHexString(msgData) + ")", e);
        }
    }
