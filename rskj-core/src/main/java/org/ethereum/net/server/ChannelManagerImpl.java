...
public class ChannelManagerImpl implements ChannelManager {
    ...
    public ChannelManagerImpl(RskSystemProperties config, SyncPool syncPool) {
        this.mainWorker =
                Executors.newSingleThreadScheduledExecutor(
                        target -> new Thread(target, "newPeersProcessor"));
    ...
    public void start() {
        mainWorker.scheduleWithFixedDelay(
                this::handleNewPeersAndDisconnections, 0, 1, TimeUnit.SECONDS);
    }
    ...
    private void processNewPeers() {
        ...
        newPeers.stream()
                .filter(Channel::isProtocolsInitialized)
                .forEach(c -> processNewPeer(c, processedChannels));
    ...
    private void disconnect(Channel peer, ReasonCode reason) {
        ...
        synchronized (disconnectionTimeoutsLock) {
            disconnectionsTimeouts.put(
                    peer.getInetSocketAddress().getAddress(),
    ...
    public boolean isRecentlyDisconnected(InetAddress peerAddress) {
        synchronized (disconnectionTimeoutsLock) {
            return isRecent(
                    disconnectionsTimeouts.getOrDefault(peerAddress, Instant.EPOCH), Instant.now());
        }
    }
    ...
    public Set<NodeID> broadcastBlock(@Nonnull final Block block) {
        ...
        final BlockIdentifier bi =
                new BlockIdentifier(block.getHash().getBytes(), block.getNumber());
        final EthMessage newBlock = new RskMessage(new BlockMessage(block));
        final EthMessage newBlockHashes =
                new RskMessage(new NewBlockHashesMessage(Arrays.asList(bi)));
        synchronized (activePeersLock) {
    ...
    public Set<NodeID> broadcastBlockHash(
            @Nonnull final List<BlockIdentifier> identifiers, final Set<NodeID> targets) {
        final Set<NodeID> nodesIdsBroadcastedTo = new HashSet<>();
    ...
    public Set<NodeID> broadcastTransaction(
            @Nonnull final Transaction transaction, final Set<NodeID> skip) {
    ...
    public boolean isAddressBlockAvailable(InetAddress inetAddress) {
        synchronized (activePeersLock) {
            // TODO(lsebrie): save block address in a data structure and keep updated on each
            // channel add/remove
            // TODO(lsebrie): check if we need to use a different networkCIDR for ipv6
            return activePeers.values().stream()
                            .map(
                                    ch ->
                                            new InetAddressBlock(
                                                    ch.getInetSocketAddress().getAddress(),
                                                    networkCIDR))
