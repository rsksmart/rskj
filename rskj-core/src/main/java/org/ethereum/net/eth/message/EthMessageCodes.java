...
/**
 * A list of commands for the Ethereum network protocol. <br>
...
public enum EthMessageCodes {
     ...
     * Inform a peer of it's current ethereum state. This message should be send after the initial
     ...
     * Specify one or more new blocks which have appeared on the network. To be maximally helpful,
     * nodes should inform peers of all blocks that they may not be aware of. Including hashes that
     * the sending peer could reasonably be considered to know (due to the fact they were previously
     * informed of because that node has itself advertised knowledge of the hashes through
     * NewBlockHashes) is considered Bad Form, and may reduce the reputation of the sending node.
     * Including hashes that the sending node later refuses to honour with a proceeding GetBlocks
     ...
     * queue. The items in the list (following the first item 0x12) are transactions in the format
     * described in the main Ethereum specification.
     */
    TRANSACTIONS(0x02),

    /**
     * {@code [+0x03: P, block: { P , B_32 }, maxHeaders: P, skip: P, reverse: P in { 0 , 1 } ] }
     * <br>
     * Replaces GetBlockHashes since PV 62. <br>
     * Require peer to return a BlockHeaders message. Reply must contain a number of block headers,
     * of rising number when reverse is 0, falling when 1, skip blocks apart, beginning at block
     * block (denoted by either number or hash) in the canonical chain, and with at most maxHeaders
     * items.
     */
     ...
     * Reply to GetBlockHeaders. The items in the list (following the message ID) are block headers
     * in the format described in the main Ethereum specification, previously asked for in a
     * GetBlockHeaders message. This may validly contain no block headers if no block headers were
     * able to be returned for the GetBlockHeaders query.
     */
    BLOCK_HEADERS(0x04),
     ...
     * Require peer to return a BlockBodies message. Specify the set of blocks that we're interested
     * in with the hashes.
     */
     ...
     * Reply to GetBlockBodies. The items in the list (following the message ID) are some of the
     * blocks, minus the header, in the format described in the main Ethereum specification,
     * previously asked for in a GetBlockBodies message. This may validly contain no block headers
