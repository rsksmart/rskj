...
public class EncryptionHandshake {
    ...
    public AuthInitiateMessage createAuthInitiate(@Nullable byte[] token, ECKey key) {
        ...
        message.ephemeralPublicHash = HashUtil.keccak256(ephemeralKey.getPubKeyPoint().getEncoded(false), 1, 64);
        message.publicKey = key.getPubKeyPoint();
        message.nonce = initiatorNonce;
        return message;
    }
    ...
    void agreeSecret(byte[] initiatePacket, byte[] responsePacket) {
        ...
        byte[] sharedSecret = keccak256(agreedSecret, keccak256(responderNonce, initiatorNonce));
        byte[] aesSecret = keccak256(agreedSecret, sharedSecret);
        secrets = new Secrets();
        secrets.aes = aesSecret;
        secrets.mac = keccak256(agreedSecret, aesSecret);
        secrets.token = HashUtil.keccak256(sharedSecret);
