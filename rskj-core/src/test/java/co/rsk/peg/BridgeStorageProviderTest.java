...
public class BridgeStorageProviderTest {
    ...
    private final BridgeStorageConfiguration bridgeStorageConfigurationBeforeForks =
            new BridgeStorageConfiguration(false, false);
    private final BridgeStorageConfiguration bridgeStorageConfigurationFedMultiKey =
            new BridgeStorageConfiguration(false, true);
    ...
    public void createInstance() throws IOException {
        ...
        BridgeStorageProvider provider =
                new BridgeStorageProvider(
                        ...
                        bridgeStorageConfigurationBeforeForks);
    ...
    public void createSaveAndRecreateInstance() throws IOException {
        ...
        Assert.assertNotNull(
                repository.getStorageBytes(
                        contractAddress, DataWord.valueOf("btcTxHashesAP".getBytes())));
        Assert.assertNotNull(
                repository.getStorageBytes(
                        contractAddress, DataWord.valueOf("releaseRequestQueue".getBytes())));
        Assert.assertNotNull(
                repository.getStorageBytes(
                        contractAddress, DataWord.valueOf("releaseTransactionSet".getBytes())));
        Assert.assertNotNull(
                repository.getStorageBytes(
                        contractAddress, DataWord.valueOf("rskTxsWaitingFS".getBytes())));
        Assert.assertNotNull(
                repository.getStorageBytes(
                        contractAddress, DataWord.valueOf("newFederationBtcUTXOs".getBytes())));
        Assert.assertNotNull(
                repository.getStorageBytes(
                        contractAddress, DataWord.valueOf("oldFederationBtcUTXOs".getBytes())));
    ...
    public void getNewFederation_initialVersion() throws IOException {
        ...
        when(repositoryMock.getStorageBytes(any(RskAddress.class), any(DataWord.class)))
                .then(
                        (InvocationOnMock invocation) -> {
                            ...
                            Assert.assertTrue(
                                    Arrays.equals(
                                            Hex.decode("aabbccdd"), contractAddress.getBytes()));

                            if (storageCalls.size() == 1) {
                                // First call is storage version getter
                                Assert.assertEquals(
                                        DataWord.fromString("newFederationFormatVersion"), address);
                                return new byte[0];
                            } else {
        ...
        PowerMockito.when(
                        BridgeSerializationUtils.deserializeFederationOnlyBtcKeys(
                                any(byte[].class), any(NetworkParameters.class)))
                .then(
                        (InvocationOnMock invocation) -> {
                            ...
                            // Make sure we're deserializing what just came from the repo with the
                            // correct BTC context
                ...
                .then(
                        (InvocationOnMock invocation) -> {
                            ...
                            if (storageCalls.size() == 1) {
                                ...
                                Assert.assertTrue(
                                        storageCalls.size() == 2 || storageCalls.size() == 3);
                                Assert.assertEquals(DataWord.fromString("newFederation"), address);
                                return null;
                            }
                        });
    ...
    public void saveNewFederation_preMultikey() throws IOException {
        ...
        Mockito.doAnswer(
                        (InvocationOnMock invocation) -> {
                            ...
                            // Make sure the bytes are set to the correct address in the repo and
                            // that what's saved is what was serialized
                            Assert.assertTrue(
                                    Arrays.equals(
                                            new byte[] {
                            ...
                            Assert.assertEquals(
                                    DataWord.valueOf(
                                            "newFederation".getBytes(StandardCharsets.UTF_8)),
                                    address);
                            Assert.assertTrue(Arrays.equals(new byte[] {(byte) 0xbb}, data));
                            return null;
                        })
                .when(repositoryMock)
                .addStorageBytes(any(RskAddress.class), any(DataWord.class), any(byte[].class));
    ...
    public void saveNewFederation_postMultikey() throws IOException {
        ...
        BridgeStorageProvider storageProvider =
                new BridgeStorageProvider(
                        ...
                        mockAddress("aabbccdd"),
                        config.getNetworkConstants().getBridgeConstants(),
                        bridgeStorageConfigurationFedMultiKey);

        PowerMockito.when(BridgeSerializationUtils.serializeFederation(any(Federation.class)))
                .then(
                        (InvocationOnMock invocation) -> {
                            Federation federation = invocation.getArgument(0);
                            Assert.assertEquals(newFederation, federation);
                        ...
                        (InvocationOnMock invocation) -> {
                            ...
                            if (storageBytesCalls.size() == 1) {
                                ...
                                Assert.assertEquals(
                                        DataWord.fromString("newFederationFormatVersion"), address);
                                Assert.assertEquals(
                                        BigInteger.valueOf(1000), RLP.decodeBigInteger(data, 0));
                            } else {
                                Assert.assertEquals(2, storageBytesCalls.size());
                                // Make sure the bytes are set to the correct address in the repo
                                // and that what's saved is what was serialized
                                Assert.assertTrue(
                                        Arrays.equals(
                                                Hex.decode("aabbccdd"),
                                                contractAddress.getBytes()));
                ...
                .then(
                        (InvocationOnMock invocation) -> {
                            ...
                            if (storageCalls.size() == 1) {
                                // First call is storage version getter
                                Assert.assertEquals(
                                        DataWord.fromString("oldFederationFormatVersion"), address);
                                return new byte[0];
                            } else {
                                // Second call is the actual storage getter
                                Assert.assertEquals(2, storageCalls.size());
    ...
    public void saveOldFederation_preMultikey_setToNull() throws IOException {
        ...
        Mockito.doAnswer(
                ...
                .when(repositoryMock)
                .addStorageBytes(any(RskAddress.class), any(DataWord.class), any());
                ...
                .then(
                        (InvocationOnMock invocation) -> {
                            ...
                            if (storageCalls.size() == 1) {
                                ...
                                Assert.assertEquals(2, storageCalls.size());
                                Assert.assertEquals(
                                        DataWord.fromString("pendingFederation"), address);
                                return new byte[] {(byte) 0xaa};
                            }
                        });
    ...
    public void getPendingFederation_multiKeyVersion() throws IOException {
        ...
        PowerMockito.when(BridgeSerializationUtils.deserializePendingFederation(any(byte[].class)))
                .then(
                        (InvocationOnMock invocation) -> {
                            deserializeCalls.add(0);
                            byte[] data = invocation.getArgument(0);
    ...
    public void savePendingFederation_postMultikey() throws IOException {
        ...
        PowerMockito.when(
                        BridgeSerializationUtils.serializePendingFederation(
                                any(PendingFederation.class)))
                .then(
                        (InvocationOnMock invocation) -> {
                            PendingFederation federation = invocation.getArgument(0);
        ...
        when(repositoryMock.getStorageBytes(any(RskAddress.class), any(DataWord.class)))
                .then(
                        (InvocationOnMock invocation) -> {
                            ...
                            Assert.assertEquals(
                                    DataWord.valueOf(
                                            "federationElection".getBytes(StandardCharsets.UTF_8)),
                                    address);
                            return new byte[] {(byte) 0xaa};
                        });
        PowerMockito.when(
                        BridgeSerializationUtils.deserializeElection(
                                any(byte[].class), any(AddressBasedAuthorizer.class)))
                .then(
                        (InvocationOnMock invocation) -> {
                            calls.add(0);
                            byte[] data = invocation.getArgument(0);
                            ...
                            // Make sure we're deserializing what just came from the repo with the
                            // correct AddressBasedAuthorizer
                            Assert.assertTrue(Arrays.equals(new byte[] {(byte) 0xaa}, data));
    ...
    public void saveFederationElection() throws IOException {
        ...
        PowerMockito.when(BridgeSerializationUtils.serializeElection(any(ABICallElection.class)))
                .then(
                        (InvocationOnMock invocation) -> {
    ...
    public void getLockWhitelist_nonNullBytes() {
        ...
        LockWhitelistEntry oneOffEntry =
                new OneOffWhiteListEntry(
                        getBtcAddress("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"), Coin.COIN);
        LockWhitelistEntry unlimitedEntry =
                new UnlimitedWhiteListEntry(
                        getBtcAddress("bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"));
        whitelistMock.put(oneOffEntry.address(), oneOffEntry);
        whitelistMock.put(unlimitedEntry.address(), unlimitedEntry);
        PowerMockito.mockStatic(BridgeSerializationUtils.class);
        Repository repositoryMock = mock(Repository.class);
        // Overriding BridgeStorageConfiguration to make sure it serializes the unlimited whitelist
        // data
        BridgeStorageProvider storageProvider =
                new BridgeStorageProvider(
                        repositoryMock,
        ...
        when(repositoryMock.getStorageBytes(
                        ...
                        eq(DataWord.valueOf("lockWhitelist".getBytes(StandardCharsets.UTF_8)))))
                .then(
                        (InvocationOnMock invocation) -> {
                            calls.add(0);
                            RskAddress contractAddress = invocation.getArgument(0);
                            ...
                            Assert.assertEquals(
                                    DataWord.valueOf(
                                            "lockWhitelist".getBytes(StandardCharsets.UTF_8)),
                                    address);
                            return new byte[] {(byte) 0xaa};
                        });
        when(repositoryMock.getStorageBytes(
        ...
        PowerMockito.when(
                ...
                .then(
                        (InvocationOnMock invocation) -> {
                            ...
                            // correct AddressBasedAuthorizer
                            Assert.assertTrue(
                                    Arrays.equals(new byte[] {(byte) 0xbb}, unlimitedData));
                            HashMap<Address, LockWhitelistEntry> map = new HashMap<>();
    ...
    public void saveLockWhitelist() {
        ...
        PowerMockito.when(BridgeSerializationUtils.serializeOneOffLockWhitelist(any(Pair.class)))
                .then(
                        (InvocationOnMock invocation) -> {
                            Pair<List<OneOffWhiteListEntry>, Integer> data =
                                    invocation.getArgument(0);
                            Assert.assertEquals(
                                    whitelistMock.getAll(OneOffWhiteListEntry.class),
                                    data.getLeft());
                            Assert.assertSame(
                                    whitelistMock.getDisableBlockHeight(), data.getRight());
                            serializeCalls.add(0);
        ...
        PowerMockito.when(BridgeSerializationUtils.serializeUnlimitedLockWhitelist(any(List.class)))
                .then(
                        (InvocationOnMock invocation) -> {
                            List<UnlimitedWhiteListEntry> unlimitedWhiteListEntries =
                                    invocation.getArgument(0);
                            Assert.assertEquals(
                                    whitelistMock.getAll(UnlimitedWhiteListEntry.class),
        ...
        Mockito.doAnswer(
                ...
                .addStorageBytes(
                        ...
                        eq(
                                DataWord.valueOf(
                                        "unlimitedLockWhitelist".getBytes(StandardCharsets.UTF_8))),
                        any(byte[].class));
    ...
    public void saveLockWhiteListAfterGetWithData() {
        ...
        PowerMockito.mockStatic(BridgeSerializationUtils.class);
        Repository repositoryMock = mock(Repository.class);
        OneOffWhiteListEntry oneOffEntry =
                new OneOffWhiteListEntry(
                        getBtcAddress("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"), Coin.COIN);
        BridgeStorageProvider storageProvider =
                new BridgeStorageProvider(
        ...
        when(repositoryMock.getStorageBytes(any(RskAddress.class), any(DataWord.class)))
                .then(
                        (InvocationOnMock invocation) -> {
                            ...
                            Assert.assertEquals(
                                    DataWord.valueOf(
                                            "releaseRequestQueue".getBytes(StandardCharsets.UTF_8)),
                                    address);
                            return new byte[] {(byte) 0xaa};
                        });
        PowerMockito.when(
    ...
    private BtcTransaction createTransaction() {
        ...
        tx.addInput(
                ...
                ScriptBuilder.createInputScript(
                        new TransactionSignature(BigInteger.ONE, BigInteger.TEN)));
    ...
    private Address getBtcAddress(String addr) {
        return new Address(
                config.getNetworkConstants().getBridgeConstants().getBtcParams(), Hex.decode(addr));
    }
    ...
    private void useOriginalIntegerSerialization() {
        PowerMockito.when(BridgeSerializationUtils.serializeInteger(any(Integer.class)))
                .thenCallRealMethod();
        PowerMockito.when(BridgeSerializationUtils.deserializeInteger(any(byte[].class)))
                .thenCallRealMethod();
    }
