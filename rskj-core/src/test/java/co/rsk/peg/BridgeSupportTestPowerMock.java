...
public class BridgeSupportTestPowerMock {
    ...
    private BridgeStorageProvider globalProvider;
    private Context globalBtcContext;
    ...
    public void callUpdateCollectionsFundsEnoughForJustTheSmallerTx() throws IOException {
        ...
        BridgeStorageProvider provider0 = new BridgeStorageProvider(track, PrecompiledContracts.BRIDGE_ADDR, bridgeConstants, activationsBeforeForks, pegUtils.getBridgeSerializationUtils());
    ...
    public void callUpdateCollectionsWithTransactionsWaitingForConfirmationWithEnoughConfirmations() throws IOException {
        ...
        pegUtils = spy(pegUtils);
        when(pegUtils.getBridgeUtils()).thenReturn(bridgeUtils);

        // Fake wallet returned every time
        when(bridgeUtils.getFederationSpendWallet(any(Context.class), any(Federation.class), any(List.class), anyBoolean(), any())).thenReturn(mock(Wallet.class));
        ...
        BridgeStorageProvider provider2 = new BridgeStorageProvider(repository, PrecompiledContracts.BRIDGE_ADDR, bridgeConstants, activationsBeforeForks, pegUtils.getBridgeSerializationUtils());
    ...
    public void addBlockHeaderToBlockchain() throws IOException, BlockStoreException {
        ...
        BridgeSupport bridgeSupport = getBridgeSupport(provider, track, mockFactory, activations, btcBlockChain);
        co.rsk.bitcoinj.core.BtcBlock block = new co.rsk.bitcoinj.core.BtcBlock(
                btcParams,
                1,
                PegTestUtils.createHash(1),
    ...
    public void registerBtcTransactionMigrationTx() throws BlockStoreException, AddressFormatException, IOException, BridgeIllegalArgumentException {
        ...
        Federation activeFederation = new Federation(FederationTestUtils.getFederationMembersWithBtcKeys(activeFederationKeys), Instant.ofEpochMilli(2000L), 2L, parameters, pegUtils.getScriptBuilderWrapper());
        ...
        Federation retiringFederation = new Federation(FederationTestUtils.getFederationMembersWithBtcKeys(retiringFederationKeys), Instant.ofEpochMilli(1000L), 1L, parameters, pegUtils.getScriptBuilderWrapper());
    ...
    public void registerBtcTransactionWithCrossFederationsChange() throws Exception {
        ...
        Federation activeFederation = new Federation(
                ...
                Instant.ofEpochMilli(1000L), 0L, params, pegUtils.getScriptBuilderWrapper()
        );
        ...
        Federation retiringFederation = new Federation(
                ...
                Instant.ofEpochMilli(2000L), 0L, params, pegUtils.getScriptBuilderWrapper()
        );
        ...
        BridgeUtils bridgeUtils = spy(pegUtils.getBridgeUtils());
        pegUtils = spy(pegUtils);
        when(pegUtils.getBridgeUtils()).thenReturn(bridgeUtils);

        doReturn(false).when(bridgeUtils).isValidPegInTx(
        ...
        doReturn(true).when(bridgeUtils).isMigrationTx(
                any(BtcTransaction.class),
        ...
        verify(bridgeUtils).isMigrationTx(releaseWithChangeTx, activeFederation, retiringFederation, null, btcContext, bridgeConstants, activations);
    }
    ...
    public void registerBtcTransactionLockTxWhitelisted() throws Exception {
        ...
        Federation federation1 = new Federation(FederationTestUtils.getFederationMembersWithBtcKeys(federation1Keys), Instant.ofEpochMilli(1000L), 0L, btcParams, pegUtils.getScriptBuilderWrapper());
        ...
        Federation federation2 = new Federation(FederationTestUtils.getFederationMembersWithBtcKeys(federation2Keys), Instant.ofEpochMilli(2000L), 0L, btcParams, pegUtils.getScriptBuilderWrapper());
    ...
    public void getPendingFederationMethods_present() throws IOException {
        PendingFederation mockedPendingFederation = new PendingFederation(FederationTestUtils.getFederationMembers(5), pegUtils.getBridgeSerializationUtils());
    ...
    public void createFederation_ok() throws IOException, BridgeIllegalArgumentException {
        ...
        Assert.assertTrue(Arrays.equals(
                new PendingFederation(Collections.emptyList(), pegUtils.getBridgeSerializationUtils()).getHash().getBytes(),
    ...
    public void createFederation_pendingExists() throws IOException, BridgeIllegalArgumentException {
        ...
        BridgeSupport bridgeSupport = getBridgeSupportWithMocksForFederationTests(
                ...
                null,
                null,
                new PendingFederation(Collections.emptyList(), pegUtils.getBridgeSerializationUtils()),
                mocksProvider.getElection(),
                null
        );
    ...
    public void createFederation_withPendingActivation() throws IOException, BridgeIllegalArgumentException {
        ...
        globalProvider.getOldFederationBtcUTXOs().add(mock(UTXO.class));
    ...
    public void addFederatorPublicKey_okNoKeys() throws IOException, BridgeIllegalArgumentException {
        ...
        PendingFederation pendingFederation = new PendingFederation(Collections.emptyList(), pegUtils.getBridgeSerializationUtils());
        BridgeSupport bridgeSupport = getBridgeSupportWithMocksForFederationTests(
                false,
                null,
                null,
    ...
    public void commitFederation_ok() throws IOException, BridgeIllegalArgumentException {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            globalProvider.getNewFederationBtcUTXOs().add(utxoMock);
        }

        // Currently active federation
        Federation oldActiveFederation = globalProvider.getNewFederation();
        Assert.assertNotNull(oldActiveFederation);

        // Vote with no winner
        Assert.assertNotNull(globalProvider.getPendingFederation());
        Assert.assertEquals(1, mocksProvider.execute(bridgeSupport));
        Assert.assertNotNull(globalProvider.getPendingFederation());

        Assert.assertEquals(oldActiveFederation, globalProvider.getNewFederation());
        Assert.assertNull(globalProvider.getOldFederation());

        // Vote with winner
        mocksProvider.setWinner(mocksProvider.getSpec());
        Assert.assertEquals(1, mocksProvider.execute(bridgeSupport));

        Assert.assertNull(globalProvider.getPendingFederation());

        Federation retiringFederation = globalProvider.getOldFederation();
        Federation activeFederation = globalProvider.getNewFederation();

        Assert.assertEquals(expectedFederation, activeFederation);
        Assert.assertEquals(retiringFederation, oldActiveFederation);

        Assert.assertEquals(0, globalProvider.getNewFederationBtcUTXOs().size());
        Assert.assertEquals(5, globalProvider.getOldFederationBtcUTXOs().size());
        for (int i = 0; i < 5; i++) {
            Assert.assertEquals(i, globalProvider.getOldFederationBtcUTXOs().get(i).getIndex());
            Assert.assertEquals(Coin.valueOf((i + 1) * 1000), globalProvider.getOldFederationBtcUTXOs().get(i).getValue());
        }
        verify(mocksProvider.getElection(), times(1)).clearWinners();
        verify(mocksProvider.getElection(), times(1)).clear();
    ...
    public void getActiveFederationWallet() throws IOException {
        ...
        Object expectedUtxos = globalProvider.getNewFederationBtcUTXOs();

        final Wallet expectedWallet = mock(Wallet.class);
        when(bridgeUtils.getFederationSpendWallet(any(), any(), any(), anyBoolean(), any())).then((InvocationOnMock m) -> {
            Assert.assertEquals(m.<Context>getArgument(0), globalBtcContext);
    ...
    public void getRetiringFederationWallet_nonEmpty() throws IOException {
        ...
        Object expectedUtxos = globalProvider.getOldFederationBtcUTXOs();
    ...
    public void getBtcTransactionConfirmations_inexistentBlockHash() throws BlockStoreException, IOException {
        ...
        BridgeStorageProvider provider = new BridgeStorageProvider(
                ...
                PrecompiledContracts.BRIDGE_ADDR,
                bridgeConstants,
                activationsBeforeForks, pegUtils.getBridgeSerializationUtils()
        );
    ...
    private BridgeSupport getBridgeSupport(
            ...
            Repository track,
            BtcBlockStoreWithCache.Factory blockStoreFactory,
            ActivationConfig.ForBlock activations, BtcBlockChain btcBlockChain) {
    ...
    private BridgeSupport getBridgeSupport(
        ...
        globalBtcContext = new Context(constants.getBtcParams());
