...
class BridgeSupportTest {
    ...
    private static final RskAddress contractAddress = BRIDGE_ADDR;
    ...
    class UnionBridgeTest {
        ...
        void requestUnionRbtc_whenFail_shouldReturnFailingResponseCode(UnionResponseCode expectedUnionResponseCode) {
            ...
            unionBridgeSupport = mock(UnionBridgeSupport.class);
            when(unionBridgeSupport.requestUnionRbtc(transaction, amountRequested)).thenReturn(expectedUnionResponseCode.getCode());
            bridgeSupport = bridgeSupportBuilder.withUnionBridgeSupport(unionBridgeSupport).build();

            // act
            int actualResponseCode = bridgeSupport.requestUnionBridgeRbtc(transaction, amountRequested);
            ...
            assertEquals(expectedUnionResponseCode.getCode(), actualResponseCode);
        }
        ...
        void requestUnionRbtc_whenSuccess_shouldReturnSuccessResponseCode() {
            ...
            co.rsk.core.Coin amountRequested = new co.rsk.core.Coin(weiPerEther);
            unionBridgeSupport = mock(UnionBridgeSupport.class);
            when(unionBridgeSupport.requestUnionRbtc(transaction, amountRequested)).thenReturn(UnionResponseCode.SUCCESS.getCode());

            RskAddress unionBridgeContractAddress = bridgeConstants.getUnionBridgeConstants().getAddress();
            when(unionBridgeSupport.getUnionBridgeContractAddress()).thenReturn(Optional.of(unionBridgeContractAddress));
            ...
            assertEquals(UnionResponseCode.SUCCESS.getCode(), actualResponseCode);
        }
    ...
    void when_registerBtcTransaction_sender_not_recognized_after_rskip170_lock() throws Exception {
        ...
        assertEquals(LIMIT_MONETARY_BASE.subtract(totalAmountExpectedToHaveBeenLocked), repository.getBalance(
            BRIDGE_ADDR));
    ...
    private void assertLockingCap(
        ...
        assertEquals(bridgeBalance.subtract(totalAmountExpectedToHaveBeenLocked), repository.getBalance(
            BRIDGE_ADDR));
