...
public class FederationTest {
    ...
    public void createFederation() {
        ...
        rskAddresses =
                Arrays.asList(101, 201, 301, 401, 501, 601).stream()
    ...
    public void membersImmutable() {
        ...
        try {
            federation
                    .getMembers()
                    .add(new FederationMember(new BtcECKey(), new ECKey(), new ECKey()));
        } catch (Exception e) {
            exception = true;
        }
    ...
    public void redeemScript() {
        ...
        PowerMockito.when(ScriptBuilder.createRedeemScript(any(int.class), any(List.class)))
                .thenAnswer(
                        (invocationOnMock) -> {
                            calls.add(1);
                            int numberOfSignaturesRequired =
                                    invocationOnMock.<Integer>getArgument(0);
                            ...
                            for (int i = 0; i < sortedPublicKeys.size(); i++) {
                                Assert.assertTrue(
                                        Arrays.equals(
                                                sortedPublicKeys.get(i).getPubKey(),
                                                publicKeys.get(i).getPubKey()));
                            }
                            return new Script(new byte[] {(byte) 0xaa});
                        });
        Assert.assertTrue(
                Arrays.equals(federation.getRedeemScript().getProgram(), new byte[] {(byte) 0xaa}));
        Assert.assertTrue(
                Arrays.equals(federation.getRedeemScript().getProgram(), new byte[] {(byte) 0xaa}));
    ...
    public void P2SHScript() {
        final List<Integer> calls = new ArrayList<>();
        PowerMockito.mockStatic(ScriptBuilder.class);
        PowerMockito.when(ScriptBuilder.createP2SHOutputScript(any(int.class), any(List.class)))
                .thenAnswer(
                        (invocationOnMock) -> {
                            calls.add(0);
                            int numberOfSignaturesRequired =
        ...
        Assert.assertTrue(
                Arrays.equals(federation.getP2SHScript().getProgram(), new byte[] {(byte) 0xaa}));
        Assert.assertTrue(
                Arrays.equals(federation.getP2SHScript().getProgram(), new byte[] {(byte) 0xaa}));
        // Make sure the script creation happens only once
        Assert.assertEquals(1, calls.size());
    ...
    @Test
    public void Address() {
        // Since we can't mock both Address and ScriptBuilder at the same time (due to PowerMockito
        // limitations)
        ...
        PowerMockito.when(ScriptBuilder.createP2SHOutputScript(any(int.class), any(List.class)))
                .thenAnswer(
                        (invocationOnMock) -> {
                            ...
                            return new Script(
                                    Hex.decode("a914896ed9f3446d51b5510f7f0b6ef81b2bde55140e87"));
                        });

        Assert.assertEquals(
                "2N5muMepJizJE1gR7FbHJU6CD18V3BpNF9p", federation.getAddress().toBase58());
        Assert.assertEquals(
                "2N5muMepJizJE1gR7FbHJU6CD18V3BpNF9p", federation.getAddress().toBase58());
    ...
    public void testEquals_differentMembers() {
        ...
        members.add(
                new FederationMember(
                        ...
                        ECKey.fromPrivate(BigInteger.valueOf(600)),
    ...
    public void getBtcPublicKeyIndex() {
        for (int i = 0; i < federation.getBtcPublicKeys().size(); i++) {
            Assert.assertEquals(
                    i, federation.getBtcPublicKeyIndex(sortedPublicKeys.get(i)).intValue());
        }
        Assert.assertNull(
                federation.getBtcPublicKeyIndex(BtcECKey.fromPrivate(BigInteger.valueOf(1234))));
    }
    ...
    public void hasBtcPublicKey() {
        ...
        Assert.assertFalse(
                federation.hasBtcPublicKey(BtcECKey.fromPrivate(BigInteger.valueOf(1234))));
    }
