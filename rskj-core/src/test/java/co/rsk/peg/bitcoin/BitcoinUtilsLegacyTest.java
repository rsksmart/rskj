...
class BitcoinUtilsLegacyTest {
    ...
    void getMultiSigTransactionHashWithoutSignaturesBeforeRSKIP305_fromRealPeginFromP2PKHWithParseableScriptPubKey_shouldThrowISE() {
        ...
        BtcTransaction testnetRealPegin = new BtcTransaction(testnetParams, testnetRealPeginSerialized);
    ...
    void getMultiSigTransactionHashWithoutSignaturesBeforeRSKIP305_fromP2PKH_withParseableScriptPubKey_shouldThrowISE() {
        ...
        BtcECKey parseableKey = BtcECKey.fromPublicOnly(
            Hex.decode("0377a6c71c43d9fac4343f87538cd2880cf5ebefd3dd1d9aabdbbf454bca162de9")
        );
        Script parseableScriptPubKeyInputScript = ScriptBuilder.createInputScript(null, parseableKey);
        ...
        Script inputScript = scriptBuilder
            .data(signature)
            .data(parseableScriptPubKeyInputScript.getProgram())
            .build();
        ...
        // act & assert
        int inputIndex = 0;
        Optional<Script> redeemScript = extractRedeemScriptFromInput(tx, inputIndex);
        assertTrue(redeemScript.isPresent()); // extractRedeemScriptFromInput incorrectly extracts redeem script from P2PKH input
