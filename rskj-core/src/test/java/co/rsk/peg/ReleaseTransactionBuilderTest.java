...
class ReleaseTransactionBuilderTest {
    ...
    @Test
    void createRealTransactionInMainnet() {
...
//        BtcTransaction btcTx = createRealBtcTransactionWith200InputsAnd50Outputs(myP2shP2wshErpFederation, theSigningKeys, btcMainNetParams);
...
//        System.out.println(Hex.toHexString(btcTx.bitcoinSerialize()));
    ...
    private BtcTransaction createRealBtcTransactionWith200InputsAnd50Outputs(byte[] rawPrevBtcTx, ErpFederation activeFederation, List<BtcECKey> signingKeys, NetworkParameters networkParameters, Address recipient) {
        ...
        Script p2wshP2shRedeemScript = activeFederation.getRedeemScript();
        ...
        for (int prevOutputIndex = 0; prevOutputIndex < prevBtcTx.getOutputs().size() - 1; prevOutputIndex++) {
            btcTx.addInput(spendTxHash, prevOutputIndex, segwitScriptSig);
        }

        int numberOfSignaturesRequired = activeFederation.getNumberOfSignaturesRequired();
        ...
        for (int inputIndex = 0; inputIndex < btcTx.getInputs().size(); inputIndex++) {
            ...
            Script outputScript = activeFederation.getDefaultP2SHScript();
            for (int i = 0; i < numberOfSignaturesRequired; i++) {
                BtcECKey keyToSign = signingKeys.get(i);
