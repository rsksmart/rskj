...
public class MessageDecoderTest {
    ...
    public void testDataSizePingMessage() {
        ...
        PingPeerMessage pingPeerMessage =
                PingPeerMessage.create(LOCALHOST, PORT, check, key1, NETWORK_ID);
    ...
    public void testDataSizePongMessage() {
        ...
        PongPeerMessage pongPeerMessage =
                PongPeerMessage.create(LOCALHOST, PORT, check, key1, NETWORK_ID);
    ...
    public void decode() {
        ...
        PingPeerMessage expectedPingMessage =
                PingPeerMessage.create(LOCALHOST, PORT, check, key1, NETWORK_ID);
        PingPeerMessage actualPingMessage =
                (PingPeerMessage) MessageDecoder.decode(expectedPingMessage.getPacket());
        ...
        PongPeerMessage expectedPongMessage =
                PongPeerMessage.create(LOCALHOST, PORT + 1, check, key1, NETWORK_ID);
        PongPeerMessage actualPongMessage =
                (PongPeerMessage) MessageDecoder.decode(expectedPongMessage.getPacket());
        ...
        FindNodePeerMessage expectedFindNodePeerMessage =
                FindNodePeerMessage.create(key1.getNodeId(), check, key1, NETWORK_ID);
        FindNodePeerMessage actualFindNodePeerMessage =
                (FindNodePeerMessage)
                        MessageDecoder.decode(expectedFindNodePeerMessage.getPacket());

        assertDecodedMessage(actualPingMessage, expectedPingMessage);

        Assert.assertEquals(
                actualFindNodePeerMessage.getMessageType(), DiscoveryMessageType.FIND_NODE);
        Assert.assertEquals(
                actualFindNodePeerMessage.getNodeId(), expectedFindNodePeerMessage.getNodeId());

        NeighborsPeerMessage expectedNeighborsPeerMessage =
                NeighborsPeerMessage.create(new ArrayList<>(), check, key1, NETWORK_ID);
        NeighborsPeerMessage actualNeighborsPeerMessage =
                (NeighborsPeerMessage)
                        MessageDecoder.decode(expectedNeighborsPeerMessage.getPacket());

        assertDecodedMessage(actualNeighborsPeerMessage, expectedNeighborsPeerMessage);

        Assert.assertEquals(
                actualNeighborsPeerMessage.getNodes(), expectedNeighborsPeerMessage.getNodes());
        Assert.assertEquals(
                actualNeighborsPeerMessage.getMessageType(), DiscoveryMessageType.NEIGHBORS);
    }

    public void assertDecodedMessage(
            PeerDiscoveryMessage actualMessage, PeerDiscoveryMessage expectedMessage) {
