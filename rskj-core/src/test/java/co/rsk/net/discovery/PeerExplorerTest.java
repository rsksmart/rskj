...
public class PeerExplorerTest {
    ...
    private static final String KEY_4 =
            "bd4d20e480dfb1c9c07ba0bc8cf9052f89923d38b5128c5dbfc18d4eea38264f";
    private static final String NODE_ID_4 =
            "900783f4b84b9a7c2ad44b24c62d9b6b8deed3aa428ed4a091f349229d3b7bd9dd0cd72135fe69cadc9dc1c224ee153f3467a1d292310ed29017102dc3a34b96";
    private static final String HOST_4 = "localhost";
    private static final int PORT_4 = 44038;

    //    private static final String KEY_5 =
    // "bd5d20e480dfb1c9c07ba0bc8cf9052f89923d38b5128c5dbfc18d4eea38265f";
    private static final String NODE_ID_5 =
            "55f358ce8275a895d16890c60562ee379e298174b2baa607240b07ed7e7f73e8ce3658ce74e6f677bdf9f23d176daaeb7adfe2ecc94fd21d89964b411acf6c1f";
    ...
    public void sendInitialMessageToNodesNoNodes() {
        ...
        NodeDistanceTable distanceTable =
                new NodeDistanceTable(KademliaOptions.BINS, KademliaOptions.BUCKET_SIZE, node);
    ...
    public void handlePingMessageFromDifferentNetwork() throws Exception {
        ...
        PeerExplorer peerExplorer =
                new PeerExplorer(
                        nodes, node, distanceTable, key2, TIMEOUT, UPDATE, CLEAN, NETWORK_ID1);
        ...
        PingPeerMessage pingPeerMessageWithDifferentNetwork =
                PingPeerMessage.create(HOST_1, PORT_1, check, key1, this.NETWORK_ID2);
        DiscoveryEvent incomingPingEvent =
                new DiscoveryEvent(
                        pingPeerMessageWithDifferentNetwork, new InetSocketAddress(HOST_1, PORT_1));
    ...
    public void handlePingMessage() throws Exception {
        ...
        PingPeerMessage nodeMessage =
                PingPeerMessage.create(HOST_1, PORT_1, check, key1, this.NETWORK_ID1);
        DiscoveryEvent incomingPingEvent =
                new DiscoveryEvent(nodeMessage, new InetSocketAddress(HOST_1, PORT_1));
        ...
        // After a pong returns from a node, when we receive a ping from that node, we only answer
        // with a pong (no additional ping)
        PongPeerMessage pongResponseFromSender =
                PongPeerMessage.create(
                        HOST_1, PORT_1, toSenderPing.getMessageId(), key1, NETWORK_ID1);
        DiscoveryEvent incomingPongEvent =
                new DiscoveryEvent(pongResponseFromSender, new InetSocketAddress(HOST_1, PORT_1));
    ...
    public void handlePongMessage() throws Exception {
        ...
        DiscoveryEvent incomingPongEvent =
                new DiscoveryEvent(incomingPongMessage, new InetSocketAddress(HOST_1, PORT_1));
        channel.clearEvents();
        channel.channelRead0(ctx, incomingPongEvent);
        ...
        incomingPongMessage =
                PongPeerMessage.create(
                        HOST_1,
                        PORT_1,
                        ((PingPeerMessage) initialPingMessages.get(0).getMessage()).getMessageId(),
                        key1,
                        NETWORK_ID1);
        incomingPongEvent =
                new DiscoveryEvent(incomingPongMessage, new InetSocketAddress(HOST_1, PORT_1));
        ...
        incomingPongMessage =
                PongPeerMessage.create(
                        HOST_4,
                        PORT_4,
                        ((PingPeerMessage) initialPingMessages.get(1).getMessage()).getMessageId(),
                        key4,
                        NETWORK_ID1);
        incomingPongEvent =
                new DiscoveryEvent(incomingPongMessage, new InetSocketAddress(HOST_1, PORT_1));
    ...
    public void handleFindNodeMessage() throws Exception {
        ...
        // We try to handle a findNode message from an unkown sender, no message should be send as a
        // response
        String check = UUID.randomUUID().toString();
        FindNodePeerMessage findNodePeerMessage =
                FindNodePeerMessage.create(key1.getNodeId(), check, key1, NETWORK_ID1);
        channel.clearEvents();
        channel.channelRead0(
                ctx,
                new DiscoveryEvent(findNodePeerMessage, new InetSocketAddress(HOST_1, PORT_1)));
        List<DiscoveryEvent> sentEvents = channel.getEventsWritten();
        Assert.assertEquals(0, sentEvents.size());
        ...
        PongPeerMessage incomingPongMessage =
                PongPeerMessage.create(
                        HOST_1,
                        PORT_1,
                        ((PingPeerMessage) sentEvents.get(0).getMessage()).getMessageId(),
                        key1,
                        NETWORK_ID1);
        DiscoveryEvent incomingPongEvent =
                new DiscoveryEvent(incomingPongMessage, new InetSocketAddress(HOST_1, PORT_1));
        ...
        incomingPongMessage =
                PongPeerMessage.create(
                        ...
                        NETWORK_ID1);
        incomingPongEvent =
                new DiscoveryEvent(incomingPongMessage, new InetSocketAddress(HOST_3, PORT_3));
        channel.channelRead0(ctx, incomingPongEvent);
        ...
        NeighborsPeerMessage neighborsPeerMessage =
                (NeighborsPeerMessage) sentEvents.get(0).getMessage();
    ...
    public void handleFindNodeMessageWithExtraNodes() throws Exception {
        ...
        List<DiscoveryEvent> sentEvents = channel.getEventsWritten();
        Assert.assertEquals(2, sentEvents.size());
        PongPeerMessage incomingPongMessage1 =
                PongPeerMessage.create(
                        HOST_1,
                        PORT_1,
                        ((PingPeerMessage) sentEvents.get(0).getMessage()).getMessageId(),
                        key1,
                        NETWORK_ID1);
        DiscoveryEvent incomingPongEvent1 =
                new DiscoveryEvent(incomingPongMessage1, new InetSocketAddress(HOST_1, PORT_1));
        PongPeerMessage incomingPongMessage2 =
                PongPeerMessage.create(
                        HOST_3,
                        PORT_3,
                        ((PingPeerMessage) sentEvents.get(1).getMessage()).getMessageId(),
                        key3,
                        NETWORK_ID1);
        DiscoveryEvent incomingPongEvent2 =
                new DiscoveryEvent(incomingPongMessage2, new InetSocketAddress(HOST_3, PORT_3));
    ...
    private boolean cotainsNode(String nodeId, List<Node> nodes) {
        for (Node n : nodes) {
            if (StringUtils.equals(n.getHexId(), nodeId)) return true;
        }
        return false;
    }
    ...
    public void handleNeighbors() throws Exception {
        ...
        PeerExplorer peerExplorer =
                new PeerExplorer(
                        nodes, node2, distanceTable, key2, TIMEOUT, UPDATE, CLEAN, NETWORK_ID1);
        ...
        NeighborsPeerMessage neighborsPeerMessage =
                NeighborsPeerMessage.create(
                        newNodes, UUID.randomUUID().toString(), key1, NETWORK_ID1);
        DiscoveryEvent neighborsEvent =
                new DiscoveryEvent(neighborsPeerMessage, new InetSocketAddress(HOST_1, PORT_1));
        channel.clearEvents();
        ...
        FindNodePeerMessage findNodePeerMessage =
                (FindNodePeerMessage) channel.getEventsWritten().get(0).getMessage();
        neighborsPeerMessage =
                NeighborsPeerMessage.create(
                        newNodes, findNodePeerMessage.getMessageId(), key1, NETWORK_ID1);
        neighborsEvent =
                new DiscoveryEvent(neighborsPeerMessage, new InetSocketAddress(HOST_1, PORT_1));
        channel.clearEvents();
        channel.channelRead0(ctx, neighborsEvent);
        ...
        Assert.assertEquals(
                DiscoveryMessageType.PING, discoveryEvent.getMessage().getMessageType());
        ...
        neighborsPeerMessage =
                NeighborsPeerMessage.create(
                        newNodes, findNodePeerMessage.getMessageId(), key1, NETWORK_ID1);
        neighborsEvent =
                new DiscoveryEvent(neighborsPeerMessage, new InetSocketAddress(HOST_4, PORT_4));
        channel.clearEvents();
        channel.channelRead0(ctx, neighborsEvent);
    ...
    public void testCleanPeriod() throws InterruptedException, Exception {
        ...
        PeerExplorer peerExplorer =
                new PeerExplorer(nodes, node, distanceTable, key2, 199, UPDATE, CLEAN, NETWORK_ID1);
        ...
        incomingPongEvent =
                new DiscoveryEvent(incomingPongMessage, new InetSocketAddress(HOST_1, PORT_1));
        PongPeerMessage incomingPongMessage3 =
                PongPeerMessage.create(
                        HOST_3,
                        PORT_3,
                        ((PingPeerMessage) sentEvents.get(1).getMessage()).getMessageId(),
                        key3,
                        NETWORK_ID1);
        DiscoveryEvent incomingPongEvent3 =
                new DiscoveryEvent(incomingPongMessage3, new InetSocketAddress(HOST_3, PORT_3));
        channel.clearEvents();
        List<Node> addedNodes = peerExplorer.getNodes();
        Assert.assertEquals(0, addedNodes.size());
        channel.channelRead0(ctx, incomingPongEvent);
