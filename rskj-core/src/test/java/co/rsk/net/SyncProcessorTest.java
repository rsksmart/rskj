...
public class SyncProcessorTest {
    ...
    public void noPeers() {
        ...
        SyncProcessor processor = new SyncProcessor(blockchain, blockSyncService, getPeerScoringManager(), getChannelManager(),
                SyncConfiguration.IMMEDIATE_FOR_TESTING, new ProofOfWorkRule(config).setFallbackMiningEnabled(false),
    ...
    public void syncWithAdvancedPeerAfterTimeoutWaitingPeers() {
        ...
        SyncProcessor processor = new SyncProcessor(blockchain, blockSyncService, RskMockFactory.getPeerScoringManager(), channelManager,
                                                    syncConfiguration, new ProofOfWorkRule(config).setFallbackMiningEnabled(false),
                                                                    new BlockCompositeRule(new BlockUnclesHashValidationRule(), new BlockRootValidationRule(config)),
    ...
    public void dontSyncWithoutAdvancedPeerAfterTimeoutWaitingPeers() {
        ...
        SyncProcessor processor = new SyncProcessor(blockchain, blockSyncService, RskMockFactory.getPeerScoringManager(), getChannelManager(),
                                                    SyncConfiguration.DEFAULT, new ProofOfWorkRule(config).setFallbackMiningEnabled(false),
                                                                    new BlockCompositeRule(new BlockUnclesHashValidationRule(), new BlockRootValidationRule(config)),
...
//        ((DownloadingBodiesSyncState)processor.getSyncState()).expectBodyResponseFor(lastRequestId, sender.getPeerNodeID(), block.getHeader());
    ...
    public void processBodyResponseWithTransactionAddsToBlockchain() {
        ...
        BlockExecutor blockExecutor = new BlockExecutor(blockchain.getRepository(),
                new StateRootTranslator(new HashMapDB(), new HashMap<>()), new TrieConverter(), (tx1, txindex, coinbase, repository, block1, totalGasUsed) -> new TransactionExecutor(
                        tx1,
                        txindex,
                        block1.getCoinbase(),
                        repository,
    ...
    public void findConnectionPointBlockchainWithGenesisVsBlockchainWith100Blocks() {
        ...
        SyncProcessor processor = new SyncProcessor(blockchain, blockSyncService, RskMockFactory.getPeerScoringManager(), channelManager,
                                                    SyncConfiguration.IMMEDIATE_FOR_TESTING, new DummyBlockValidationRule(),
                                                                    new BlockCompositeRule(new BlockUnclesHashValidationRule(), new BlockRootValidationRule(config)),
