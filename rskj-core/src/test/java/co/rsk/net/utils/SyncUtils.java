...
public final class SyncUtils {
    public static int syncSetupRequests(
            long bestBlock, long currentBestBlock, SyncConfiguration syncConfiguration) {
        ...
        int chunksToDownload =
                Math.max(1, Math.min(maxSkeletonChunks, chunksToBestBlock - skippedChunks));
        return 1
                + // get status
                1
                + // check best header
                binarySearchExpectedRequests(bestBlock, currentBestBlock)
    ...
    private static int binarySearchExpectedRequests(long bestBlock, long currentBestBlock) {
        ...
        while (!connectionPointFinder.getConnectionPoint().isPresent()) {
            ...
            else connectionPointFinder.updateNotFound();
        }
