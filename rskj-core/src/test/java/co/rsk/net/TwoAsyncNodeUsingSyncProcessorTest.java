...
public class TwoAsyncNodeUsingSyncProcessorTest {
    ...
    public void buildBlockchainAndSynchronize400Blocks() throws InterruptedException {
        ...
        // sync setup
        node2.waitUntilNTasksWithTimeout(
                SyncUtils.syncSetupRequests(400, 0, SyncConfiguration.IMMEDIATE_FOR_TESTING));
        // get blocks
    ...
    public void buildBlockchainWithUnclesAndSynchronize() throws InterruptedException {
        ...
        node2.waitUntilNTasksWithTimeout(
                SyncUtils.syncSetupRequests(10, 0, SyncConfiguration.IMMEDIATE_FOR_TESTING));
        // get blocks
    ...
    public void buildBlockchainPartialAndSynchronize() throws InterruptedException {
        ...
        List<Block> blocks =
                new BlockGenerator().getBlockChain(getGenesis(), 10, 0, false, true, null);
        ...
        // find connection point
        node2.waitUntilNTasksWithTimeout(
                SyncUtils.syncSetupRequests(10, 5, SyncConfiguration.IMMEDIATE_FOR_TESTING));
        // get blocks
    ...
    public void sendNewBlock() throws InterruptedException {
        ...
        node2.receiveMessageFrom(
                node1, new NewBlockHashMessage(node1.getBestBlock().getHash().getBytes()));
    ...
    public void stopSyncingAfter5SkeletonChunks() {
        ...
        node2.sendFullStatusTo(node1);
        // sync setup
        node1.waitUntilNTasksWithTimeout(
                SyncUtils.syncSetupRequests(2030, 30, SyncConfiguration.IMMEDIATE_FOR_TESTING));
        // request bodies
    ...
    public void syncInMultipleStepsWithLongBlockchain() {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            node2.sendFullStatusTo(node1);
            node1.waitUntilNTasksWithTimeout(
                    SyncUtils.syncSetupRequests(
                            4300, currentBestBlock, SyncConfiguration.IMMEDIATE_FOR_TESTING));
