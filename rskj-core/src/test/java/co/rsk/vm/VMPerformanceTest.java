...
public class VMPerformanceTest {
    ...
    private void testVMPerformance1(ResultLogger resultLogger) {
        ...
        if (useProfiler) waitForProfiler();

        System.out.println("Configuration: shortArg =  " + shortArg.toString());

        // Program
        measureProgram(
                "PUSH/POP", Hex.decode("60A0" + "50"), 2, 2, 0, 100, null); // push "A0", pop it
    ...
    public long measureOpcode(
            OpCode opcode, Boolean reference, long refTime, ResultLogger resultLogger) {
        ...
        if (reference) name = "I" + opcode.require() + "O" + opcode.ret() + "(" + name + ")";

        // execute twice each case.
        // Both times the executed method is exactly equal. We just want to detect any huge variance
        // because of the GC interrupting.
    ...
    /**
     * This method guarantees that garbage collection is done unlike <code>{@link System#gc()}
     * </code>
     */
    ...
    public long measureProgram(
        ...
        for (int g = 0; g < maxGroups; g++) {
            ...
            pr.deltaRealTimeMillis =
                    (endRealTime - startRealTime)
                            * 1000
                            * 1000
                            / maxLoops
                            / divisor; // de milli a nano
            pr.deltaGCTimeMillis =
                    (endGCTime - startGCTime) * 1000 * 1000 / maxLoops / divisor; // nanos
            ...
            if ((best == null) || (pr.deltaTime_nS < best.deltaTime_nS)) {
                best = pr;
                if (best.deltaTime_nS == 0) System.out.println("bad time");
                pr = new PerfRes();
            }
        }
        long percent;
        if (refTime != 0) percent = (best.deltaTime_nS - refTime) * 100 / refTime;
        else percent = 0;
        ...
        System.out.println(
                padRight(opcode, 12)
                        + ":"
                        + " wcT[ms]: "
                        + padLeft(Long.toString(best.wallClockTimeMillis), spaces)
                        + "| full[ns]: "
                        + padLeft(Long.toString(best.deltaTime_nS), spaces)
                        + "| ref[ns]: "
                        + padLeft(Long.toString(best.deltaTime_nS - refTime), spaces)
                        + "| (%ref): "
                        + padLeft(Long.toString(percent), 5)
                        + "| gas: "
                        + padLeft(Long.toString(best.gas), 5)
                        + "| blkTime[ms]: "
                        + padLeft(Long.toString(blockTime), spaces)
                        + "| T/gas[ns]: "
                        + padLeft(nsPerGasUnit, spaces)
                        + "| real[ns]: "
                        + padLeft(Long.toString(best.deltaRealTimeMillis), spaces)
                        + "| gcT[ns]: "
                        + padLeft(Long.toString(best.deltaGCTimeMillis), spaces)
                        + "| memPerBlk[Kb]: "
                        + padLeft(
                                Long.toString(Math.round(best.deltaUsedMemory * memToBlkMem)),
                                spaces + 1));
        ...
        Assert.assertTrue(
                memPerBlockMegabytes < PerformanceTestConstants.maxMegabytesConsumedPerBlock);
    ...
    void endMeasure() {
        if (startTime != 0) {
            ...
            // System.out.println("Time elapsed [us]: " + Long.toString(deltaTime_nS)+" [s]:"+
            // Long.toString(deltaTime_nS/1000/1000));
            System.out.println(
                    "Time elapsed [ms]: "
                            + Long.toString(deltaTime / 1000)
                            + " [s]:"
                            + Long.toString(deltaTime / 1000 / 1000));
        }
        ...
        if (startRealTime != 0) {
            ...
            System.out.println(
                    "RealTime elapsed [ms]: "
                            + Long.toString(deltaRealTime)
                            + " [s]:"
                            + Long.toString(deltaRealTime / 1000));
        }
        long endGCTime = getGarbageCollectorTimeMillis();
        long deltaGCTime = (endGCTime - startGCTime);
        System.out.println(
                "GCTime elapsed [ms]: "
                        + Long.toString(deltaGCTime)
                        + " [s]:"
                        + Long.toString(deltaGCTime / 1000));
    }
    ...
    public void testLongOperation() {
        ...
        // String s_expected_1 = "000000000000000000000000000000000000000000000000000000033FFC1244";
        // // 55
        // String s_expected_1 =
        // "00000000000000000000000000000000000000000000000000000002EE333961";// 50
        String s_expected_1 =
                "0000000000000000000090A7ED63052BFF49E105B6B7BC90D0B352C89BA1AD59"; // 254
    ...
    public void testFibonacciLongTime() {
         ...
         *
         * <p>contract Fibonacci50Times { function() { uint256 a; uint256 b; uint256 c; for (uint k
         * = 0; k < 1000; k++) { // The maximum value of k was varied a=0; b=1; for (uint i = 1; i <
         * 50; i++) { c = a+b; a = b; b = c; } } assembly { mstore(0x0, b) return(0x0, 32) } } } //
         * contract
         * ******************************************************************************************
         */
        vm = new VM(config.getVmConfig(), new PrecompiledContracts(config, null));
        ...
        // max k=100
        // byte[] codePlusPrefix =
        // Hex.decode("");
        // max k=1000
        // byte[] codePlusPrefix =
        // Hex.decode("");
        // max k=10K
        //  byte[] codePlusPrefix =
        // Hex.decode("");
        //  max k=100K
        /////////////////////////////////////////////////////////////////////////////////////////////////
        byte[] codePlusPrefix =
                Hex.decode(
                        "");
        byte[] code = Arrays.copyOfRange(codePlusPrefix, 16, codePlusPrefix.length);
    ...
    /**
     * **************** RESULTS 30/12/2016 ****************************************** Creating a
     * large set of linked memory objects to force GC... Creating 10000000 linked objects.. done.
     * Starting test.... Configuration: Program.useDataWordPool = true Time elapsed [ms]: 10062
     * [s]:10 RealTime elapsed [ms]: 10263 [s]:10 GCTime elapsed [ms]: 0 [s]:0 Instructions
     * executed: : 170400032 Instructions per second: 16934898 Avg Time per instructions [us]: 0 Avg
     * Time per instructions [ns]: 59
     * ----------------------------------------------------------------------------- Starting
     * test.... Configuration: Program.useDataWordPool = false Time elapsed [ms]: 16957 [s]:16
     * RealTime elapsed [ms]: 28273 [s]:28 GCTime elapsed [ms]: 10868 [s]:10 Instructions executed:
     * : 170400032 Instructions per second: 10048766 Avg Time per instructions [us]: 0 Avg Time per
     * instructions [ns]: 99
     * -----------------------------------------------------------------------------
     */
    public void testRunTime(byte[] code, String s_expected) {
        ...
        System.out.println(
                "Average Gas per instruction: "
                        + Long.toString(program.getResult().getGasUsed() / vm.getVmCounter()));
        ...
        System.out.println(
                "Avg Time per instructions [ns]: " + Long.toString(1000 * M / insPerSecond));
