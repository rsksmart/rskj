...
class Rskip555Eip2200Tests extends AbstractEvmTester {
    ...
    @Test
    void emptySlotToValue() {
        initSstoreContract(332211);
        Program program;
        program = executeSmartContract();

        assertEquals(GasCost.SSTORE_SET_GAS, deductSstoreGas(program, 1));
        assertEquals(DataWord.valueOf(332211), readSmartContractSlot());

        // Pre RSKIP555 behaviour
        setSmartContractSlot(0);
        initActivationConfig(false);
    ...
    @Test
    void emptySlotToValue_nested() {
        initCallableContract();
        initCallerContract(332211);

        Program program;
        program = executeSmartContract();

        assertEquals(GasCost.SSTORE_SET_GAS, deductCallerCodeGas(program.getResult().getGasUsed(), 1));
        assertEquals(DataWord.valueOf(332211), readTargetSlotValue(invoke.getNestedContractAddress()));

        // Pre RSKIP555 behaviour
        setTargetSlotValue(invoke.getNestedContractAddress(), DataWord.ZERO);
        initActivationConfig(false);
