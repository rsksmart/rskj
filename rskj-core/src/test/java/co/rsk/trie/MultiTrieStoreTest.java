...
public class MultiTrieStoreTest {
    @Test
    public void openJustStore0OnEpoch0() {
        TrieStoreFactory storeFactory = mock(TrieStoreFactory.class);
        new MultiTrieStore(0, 3, storeFactory, null);

        verify(storeFactory).newInstance("0");
        verify(storeFactory, never()).newInstance("1");
    }
    ...
    public void openLastThreeEpochsStores() {
        ...
        verify(storeFactory).newInstance("49");
    }
    ...
    public void callsSaveOnlyOnNewestStore() {
        ...
        TrieStoreFactory storeFactory = mock(TrieStoreFactory.class);
        when(storeFactory.newInstance("47")).thenReturn(store1);
        when(storeFactory.newInstance("48")).thenReturn(store2);
        when(storeFactory.newInstance("49")).thenReturn(store3);
    ...
    @Test
    public void openStore1OnEpoch0Collection() {
        TrieStoreFactory storeFactory = mock(TrieStoreFactory.class);
        MultiTrieStore store = new MultiTrieStore(0, 3, storeFactory, null);

        verify(storeFactory).newInstance("0");

        byte[] dummyHash = new byte[]{0x3, 0x9};
        store.collect(dummyHash);
        verify(storeFactory).newInstance("1");
        verify(storeFactory, never()).newInstance("2");
    }
    ...
    public void performsStoreRotationOnCollect() {
        ...
        when(storeFactory.newInstance("48")).thenReturn(store2);
        when(storeFactory.newInstance("49")).thenReturn(store3);
        when(storeFactory.newInstance("50")).thenReturn(store4);
