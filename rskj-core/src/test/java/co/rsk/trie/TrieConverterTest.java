...
public class TrieConverterTest {
    ...
    private static byte[] SERIALIZED_ORCHID_TRIESTORE_WITH_LEADING_ZEROES_STORAGE_KEYS =
            Hex.decode(
                    "0000b5e35460d"
    ...
    private static byte[] SERIALIZED_ORCHID_TRIESTORE_SIMPLE =
            Hex.decode(
                    "00003513db97e8e9afff71727bce4b2ac01ff3b"
        ...
        for (int i = 0; i < maxAccounts; i++) {
            ...
            if (i >= maxAccounts / 2) {
                ...
                for (int s = 0; s < maxStorageRows / 2; s++) {
                    track.addStorageBytes(
                            addr,
                            TestUtils.randomDataWord(),
                            TestUtils.randomBytes(TestUtils.getRandom().nextInt(40) + 1));
                }
                for (int s = 0; s < maxStorageRows / 2; s++) {
                    track.addStorageBytes(
                            addr,
                            DataWord.valueOf(TestUtils.randomBytes(20)),
                            TestUtils.randomBytes(TestUtils.getRandom().nextInt(40) + 1));
                }
    ...
    private static TrieStore trieStoreDeserialization(
