...
public class BlockChainImplTest {
    ...
    public void addValidMGPBlock() {
        ...
        BlockExecutor executor = new BlockExecutor(repository, new StateRootTranslator(new HashMapDB(), new HashMap<>()), new TrieConverter(), (tx1, txindex1, coinbase, track1, block1, totalGasUsed1) -> new TransactionExecutor(
             tx1,
             txindex1,
             block1.getCoinbase(),
             track1,
    ...
    private static BlockChainImpl createBlockChain(Repository repository, IndexedBlockStore blockStore, BlockValidatorImpl blockValidator, BlockExecutorTest.SimpleEthereumListener listener) {
        ...
        return new BlockChainImpl(repository, blockStore, receiptStore, transactionPool, listener, blockValidator, false, 1,
              new BlockExecutor(repository, new StateRootTranslator(new HashMapDB(), new HashMap<>()), new TrieConverter(), (tx1, txindex1, coinbase, track1, block1, totalGasUsed1) -> new TransactionExecutor(
                      tx1,
                      txindex1,
                      block1.getCoinbase(),
                      track1,
    ...
    private static BlockExecutor createExecutor(BlockChainImpl blockChain, BlockExecutorTest.SimpleEthereumListener listener) {
        final ProgramInvokeFactoryImpl programInvokeFactory = new ProgramInvokeFactoryImpl();
        return new BlockExecutor(blockChain.getRepository(), new StateRootTranslator(new HashMapDB(), new HashMap<>()), new TrieConverter(), (tx1, txindex1, coinbase, track1, block1, totalGasUsed1) -> new TransactionExecutor(
           tx1,
           txindex1,
           block1.getCoinbase(),
           track1,
