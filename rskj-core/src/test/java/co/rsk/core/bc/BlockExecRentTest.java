...
public class BlockExecRentTest {
    ...
    public void executeBlockWithOneTransaction() {
        ...
        System.out.println("\nSender Bal " + repository.getBalance(account,false));
        ...
        AccountState accountState = finalRepository.getAccountState(account,false );
    ...
    public void executeBlockWithOneCreateTransaction() {
        ...
        AccountState contractState = finalRepository.getAccountState(contractAddr,false);
    ...
    private Block getBlockWithOneDataTransaction() {
        ...
        // then we create the new block to connect
        List<Transaction> txs = Collections.singletonList(
                createTxWithData(account, account2, BigInteger.TEN, repository.getNonce(account.getAddress(),false), txData)
        );
    ...
    private Block getBlockWithOneCreateTransaction() {
        ...
        // then we create the new block to connect
        List<Transaction> txs = Collections.singletonList(
                createTxNullAddr(account, BigInteger.TEN, repository.getNonce(account.getAddress(),false), codeHex)
        );
