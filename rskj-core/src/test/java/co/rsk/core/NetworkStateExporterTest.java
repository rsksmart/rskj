...
public class NetworkStateExporterTest {
    ...
    public void testEmptyRepo() throws Exception {
        Repository repository = new RepositoryImpl(new TrieImpl(new TrieStoreImpl(new HashMapDB(), new TrieSerializer()), true), new HashMapDB(), new TrieStorePoolOnMemory(), config.detailsInMemoryStorageLimit());
    ...
    public void testContracts() throws Exception {
        ...
        Repository repository = new RepositoryImpl(new TrieImpl(new TrieStoreImpl(new HashMapDB(), new TrieSerializer()), true), new HashMapDB(), trieStorePool, config.detailsInMemoryStorageLimit());
        String address1String = "1000000000000000000000000000000000000000";
        RskAddress addr1 = new RskAddress(address1String);
        repository.createAccount(addr1);
        repository.addBalance(addr1, Coin.valueOf(1L));
        ...
        ContractDetails contractDetails = new co.rsk.db.ContractDetailsImpl(
            addr1.getBytes(),
            new TrieImpl(new TrieStoreImpl(new HashMapDB(), new TrieSerializer()), true),
            new byte[] {1, 2, 3, 4},
            trieStorePool,
