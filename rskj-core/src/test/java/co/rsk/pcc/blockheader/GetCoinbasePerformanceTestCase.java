...
public class GetCoinbasePerformanceTestCase extends PrecompiledContractPerformanceTestCase {
    ...
    public void getCoinbase() throws IOException {
        ...
        EnvironmentBuilder environmentBuilder =
                (int executionIndex, TxBuilder txBuilder, int height) -> {
                    ...
                    BlockHeaderContract contract =
                            new BlockHeaderContract(
                                    activationConfig,
                                    new RskAddress("0000000000000000000000000000000001000010"));
                    contract.init(
    ...
    private World buildWorld(int numBlocks, int txPerBlock, int unclesPerBlock) {
        ...
        for (int i = 0; i < numBlocks; i++) {
            Block block =
                    mineBlock(world.getBlockChain().getBestBlock(), txPerBlock, unclesPerBlock);
    ...
    private Block mineBlock(Block parent, int txPerBlock, int unclesPerBlock) {
        ...
        BtcTransaction mergedMiningCoinbaseTransaction =
                MinerUtils.getBitcoinMergedMiningCoinbaseTransaction(
                        networkParameters, mergedMiningHash.getBytes());
        ...
        byte[] additionalTag =
                Arrays.concatenate(
                        new byte[] {'A', 'L', 'T', 'B', 'L', 'O', 'C', 'K', ':'},
                        mergedMiningHash.getBytes());
        byte[] mergedMiningTx =
                org.bouncycastle.util.Arrays.concatenate(
                        compressedTag, mergedMiningHash.getBytes(), additionalTag);
