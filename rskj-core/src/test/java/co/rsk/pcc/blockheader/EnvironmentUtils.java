...
public class EnvironmentUtils {
    ...
    public static ExecutionEnvironment getEnvironmentWithBlockchainOfLength(int blockchainLength) {
        ...
        when(executionEnvironment.getBlockStore())
                .thenReturn(world.getBlockChain().getBlockStore());
        when(executionEnvironment.getBlock()).thenReturn(world.getBlockChain().getBestBlock());

        return executionEnvironment;
    }

    private static void buildBlockchainOfLength(
            World world, TestSystemProperties config, int length) {
        if (length < 0) return;

        for (int i = 0; i < length; i++) {
            Block block =
                    mineBlockWithCoinbaseTransaction(config, world.getBlockChain().getBestBlock());
            world.getBlockChain().tryToConnect(block);
        }
    }

    private static Block mineBlockWithCoinbaseTransaction(
            TestSystemProperties config, Block parent) {
        BlockGenerator blockGenerator =
                new BlockGenerator(config.getNetworkConstants(), config.getActivationConfig());
        byte[] prefix = new byte[1000];
        byte[] compressedTag =
                org.bouncycastle.util.Arrays.concatenate(prefix, RskMiningConstants.RSK_TAG);
        ...
        byte[] mergedMiningTx =
                org.bouncycastle.util.Arrays.concatenate(
                        compressedTag, blockMergedMiningHash.getBytes(), additionalTag);
