...
class Secp256k1MultiplicationTest {
    ...
    void shouldReturnCorrectMaxInputSize() {
        // when
        int maxInput = secp256k1Multiplication.getMaxInput();

        // then
        assertEquals(96, maxInput,
                "Secp256k1Multiplication should have a max input size of 96 bytes (3 × 32-byte values: x, y, scalar)");
    }

    @Test
    void shouldHaveConsistentMaxInputAndGasCost() {
        // given
        byte[] maxSizeInput = new byte[96];
        Arrays.fill(maxSizeInput, (byte) 0x01); // Fill with test data

        // when
        long gasCost = secp256k1Multiplication.getGasForData(maxSizeInput);

        // then
        assertEquals(3000L, gasCost, "Gas cost should be consistent for maximum input size");
        assertEquals(96, maxSizeInput.length, "Input size should match the expected format for EC point and scalar");
    }
}