...
class EthModuleGasEstimationDSLTest {
     ...
     *
     * This is a forward-compatibility guard rail. If the estimator is later
     * rewritten to do binary search à la geth's eth/gasestimator, this test
     * must keep passing —
     ...
     * estimator never returns a value that OOGs on replay. It guards the
     * contract that EthModule.estimateGas treats the caller's gas as a *cap
     * on the result*, not a budget for the simulation — and detects
     * OutOfGasException in the simulation result rather than returning the
     * exhausted budget as the estimate.
     *
    ...
    void testEstimateGas_proxyConstructorDelegatecallToInitializer_onchainBytecode()
        ...
        String patchedTxInput = TESTNET_PROXY_TX_INPUT.replace(onchainImpl, implAddrHex);
        assertNotEquals(TESTNET_PROXY_TX_INPUT, patchedTxInput,
        ...
        // Empirically observed true minimum for this real-world payload
        // (testnet tx 0xddb735...face96); the estimate must never come back
        // below this.
        long trueMinimum = 1_030_362L;

        long estimatedGas = estimateGas(eth, args, BlockTag.LATEST.getTag());
        ProgramResult sim = eth.getEstimationResult();
        assertTrue(estimatedGas >= trueMinimum,
                "estimate must be at least the true minimum (" + trueMinimum + ")");
        ...
        // hint to eth_estimateGas. Walk a range of caller-provided budgets that
        // straddle the true minimum (the last three are below it — 961_818 is
        // Foundry's actual submitted value) and assert the returned estimate is
        // (a) always a viable gasLimit on replay, and (b) independent of the hint.
        long[] callerHints = new long[]{
                BLOCK_GAS_LIMIT, 3_000_000L, 1_500_000L, 1_100_000L,
                trueMinimum, 961_818L, 800_000L, 740_168L
        };
        long maxNonViable = 0;
        long firstEstimate = -1;
        for (long hint : callerHints) {
            args.setGas(HexUtils.toQuantityJsonHex(hint));
            long est;
            try {
                est = estimateGas(eth, args, BlockTag.LATEST.getTag());
            } catch (Exception e) {
                // A thrown estimate is just as broken as an under-sized one —
                // never let a throw masquerade as a passing hint.
                throw new AssertionError("estimateGas threw for caller hint " + hint, e);
            }

            // The estimate must not depend on the caller's hint.
            if (firstEstimate < 0) {
                firstEstimate = est;
            } else {
                assertEquals(firstEstimate, est,
                        "estimate must be independent of the caller's gas hint (hint=" + hint + ")");
            }

            // ...and must be viable as the transaction's gasLimit on replay.
            args.setGas(HexUtils.toQuantityJsonHex(est));
            if (!runWithArgumentsAndBlock(eth, args, block)) {
     ...
     */
    private static final String TESTNET_PROXY_TX_INPUT =
