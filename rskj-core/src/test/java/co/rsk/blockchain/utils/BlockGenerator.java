...
public class BlockGenerator {
    ...
    private Genesis getNewGenesisBlock(
            long initialGasLimit, Map<RskAddress, AccountState> accounts, byte[] difficulty) {
    ...
    public Block createChildBlock(
            Block parent, long fees, List<BlockHeader> uncles, byte[] difficulty) {
        ...
        return blockFactory.newBlock(
                blockFactory.newHeader(
                        ...
                        ByteUtils.clone(parent.getStateRoot()),
                        EMPTY_TRIE_HASH,
                        EMPTY_TRIE_HASH,
    ...
    public Block createChildBlock(
            Block parent, List<Transaction> txs, byte[] stateRoot, byte[] coinbase) {
    ...
    public Block createChildBlock(
        ...
        BlockHeader newHeader =
                blockFactory.newHeader(
                        ...
                        parent.getNumber() + 1
                                        > MiningConfig
                                                .REQUIRED_NUMBER_OF_BLOCKS_FOR_FORK_DETECTION_CALCULATION
                                ? new byte[12]
                                : new byte[0],
        ...
        if (difficulty == 0) {
            newHeader.setDifficulty(
                    difficultyCalculator.calcDifficulty(newHeader, parent.getHeader()));
        } else {
    ...
    public Block createChildBlock(
            ...
            byte[] gasLimit) {
        return createChildBlock(
                parent, txs, uncles, difficulty, minGasPrice, gasLimit, parent.getCoinbase());
    }
    ...
    public Block createBlock(int number, int ntxs) {
        ...
        Coin previousMGP =
                parent.getMinimumGasPrice() != null
                        ? parent.getMinimumGasPrice()
                        : Coin.valueOf(10L);
        Coin minimumGasPrice =
                new MinimumGasPriceCalculator(Coin.valueOf(100L)).calculate(previousMGP);
    ...
    public List<Block> getBlockChain(
            Block parent, int size, int ntxs, boolean withUncles, Long difficulty) {
    ...
    public List<Block> getBlockChain(
        ...
        while (chainSize < size) {
            ...
            Block newblock =
                    createChildBlock(parent, txs, uncles, difficulty, BigInteger.valueOf(1));
    ...
    public Block getNewGenesisBlock(long initialGasLimit, Map<byte[], BigInteger> preMineMap) {
        ...
        for (Map.Entry<byte[], BigInteger> accountEntry : preMineMap.entrySet()) {
            AccountState acctState =
                    new AccountState(BigInteger.valueOf(0), new Coin(accountEntry.getValue()));
