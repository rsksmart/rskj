    ...
    public static List<String> valid(
        ...
        if (expectedState == null) {
            String formattedString =
                    String.format("Account: %s: unexpected account in the repository", addr);
            results.add(formattedString);
            return results;
        }
        ...
        if (!Arrays.equals(expectedRepository.getCode(addr), currentRepository.getCode(addr))) {
            String formattedString =
                    String.format(
                            "Account: %s: has unexpected code, expected code: %s found code: %s",
                            addr,
                            Hex.toHexString(expectedRepository.getCode(addr)),
                            Hex.toHexString(currentRepository.getCode(addr)));
            results.add(formattedString);
        }
        ...
        while (currentKeys.hasNext()) {
            ...
            if (expectedValue == null) {
                ...
                String formattedString =
                        String.format(
                                "Account: %s: has unexpected storage data: %s = %s",
                                addr, key, currentValue);
        ...
        while (expectedKeys.hasNext()) {
            ...
            if (!checked.contains(key)) {
                String formattedString =
                        String.format(
                                "Account: %s: doesn't exist. Expected storage key: %s",
                                addr, key.toString());
                results.add(formattedString);
            }
        }
