...
public class IndexedBlockStoreTest {
    ...
    @Ignore
    public void test1() {
        IndexedBlockStore indexedBlockStore =
        ...
        assertEquals(
                cumDifficulty,
                indexedBlockStore.getTotalDifficultyForHash(
                        blocks.get(blocks.size() - 1).getHash().getBytes()));
        ...
        BlockInformation blockInformation =
                indexedBlockStore.getBlocksInformationByNumber(block.getNumber()).get(0);
        ...
        block = blocks.get(8003);
        List<byte[]> hashList =
                indexedBlockStore.getListHashesEndWith(block.getHash().getBytes(), 100);
        for (int i = 0; i < 100; ++i) {
    ...
    public void test4() throws IOException {
        ...
        IndexedBlockStore indexedBlockStore =
                new IndexedBlockStore(blockFactory, indexMap, blocksDB, indexDB);
    ...
    public void test6() throws IOException {
        ...
        try {
            ...
            Block genesis = RskTestFactory.getGenesisInstance(config);
            List<Block> bestLine =
                    getRandomChain(blockFactory, genesis.getHash().getBytes(), 1, 100);
            ...
            List<Block> forkLine =
                    getRandomChain(blockFactory, forkParentHash, forkParentNumber + 1, 50);

            for (int i = 0; i < forkLine.size(); ++i) {
                Block newBlock = forkLine.get(i);
                Block parentBlock =
                        indexedBlockStore.getBlockByHash(newBlock.getParentHash().getBytes());
            ...
            for (Keccak256 hash : tDiffs.keySet()) {
                ...
                BlockDifficulty checkTd =
                        indexedBlockStore.getTotalDifficultyForHash(hash.getBytes());
            ...
            BlockDifficulty totalDifficulty =
                    indexedBlockStore.getTotalDifficultyForHash(bestBlock.getHash().getBytes());
    ...
    public void test8() throws IOException {
        ...
        try {
            ...
            long forkParentNumber = bestLine.get(60).getNumber();
            List<Block> forkLine =
                    getRandomChain(blockFactory, forkParentHash, forkParentNumber + 1, 10);
    ...
    public void test9() {
        ...
        assertEquals(
                ...
                indexedBlockStore.getTotalDifficultyForHash(block1.getHash().getBytes()));
        assertEquals(
                block2.getCumulativeDifficulty(),
                indexedBlockStore.getTotalDifficultyForHash(block2.getHash().getBytes()));
    }
}
