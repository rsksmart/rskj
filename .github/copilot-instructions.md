...
- **Dependencies are discouraged.** Any added or upgraded dependency must be accompanied by a matching PR to the `rsksmart/reproducible-builds` repository so the downloaded binary SHA256 can be verified against an independent compile. Flag dependency changes that lack this companion PR.
- **PR template compliance.** PRs must populate every section of `.github/pull_request_template.md`: **Description**, **Motivation and Context**, **How Has This Been Tested?**, **Types of changes**, **Checklist**. The checklist contains a deliberate "Requires Activation Code (Hard Fork)" question — flag PRs that touch consensus, validators, VM, peg, mining, or activation logic without answering it.
