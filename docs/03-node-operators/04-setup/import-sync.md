...
- **Disk for temporary files.** RSKj downloads the bootstrap archive and extracts it into the JVM's temporary directory (`/tmp` on Linux and macOS by default), and both exist there at the same time. The extracted contents are larger than the archive, so allow **about three times the size of the bootstrap archive** in temporary space, on top of the database itself. To check that size before committing to the download, see [Check where import sync will land you](#check-where-import-sync-will-land-you).
...
| `Error downloading bootstrap data from <url>. Please start again the import process` | The archive could not be retrieved. Retry; if it persists, the published archive may be missing from the location its index advertises. |
| `The file is corrupted or incomplete. Please start again the import process` | The archive downloaded and matched its hash, but could not be unpacked. Remove the temporary files and retry; if it persists, the published archive is faulty — report it rather than working around it. |
