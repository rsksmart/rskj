...
public class StateSnapshotImpl implements StateSnapshot {
    ...
    @Override
    public Optional<Account> findAccount(Address address) {
        return trie.find(trieMapper.addressToAccountKey(address)).map(trieMapper::valueToAccount);
    }

    @Override
    public Optional<EvmBytecode> findCode(Account account) {
        return trie.find(trieMapper.accountToCodeKey(account)).map(trieMapper::valueToCode);
    }
